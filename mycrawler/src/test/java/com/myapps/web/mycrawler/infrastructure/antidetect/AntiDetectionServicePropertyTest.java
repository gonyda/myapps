 ...
 *
 * <p>jqwik을 사용하여 randomUserAgent() 호출 결과가 항상
 * 사전 정의된 User-Agent 목록의 원소임을 검증하고,
 * 랜덤 딜레이 메서드들이 정해진 범위 내의 값만 반환하며,
 * randomViewport()가 허용된 해상도 범위 내의 값만 반환함을 검증합니다.
 *
...
class AntiDetectionServicePropertyTest {
     ...
     *
     * <p>여러 번의 호출에 대해 반환값이 항상 허용된 목록에 포함되어 있는지 확인하여,
     * 목록 외부의 값이 반환되는 경우가 없음을 보장합니다.
     *
     * @param seed 랜덤 시드로 활용되는 임의의 정수 (jqwik이 다양한 실행 경로를 탐색하도록 유도)
     */
    @Property(tries = 100)
    void randomUserAgentMustBeInPredefinedList(@ForAll @IntRange(min = 0, max = 1000) final int seed) {
     ...
     */
    @Property(tries = 100)
    void randomInterTargetDelayMustBeWithinRange(@ForAll @IntRange(min = 0, max = 1000) final int seed) {
