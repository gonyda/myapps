...
class CrawlerServicePropertyTest {
     ...
     *
     * <p>리플렉션을 사용하여 running AtomicBoolean을 true로 설정한 뒤, executeSingle() 호출 결과가 null이고
     * crawlerEngine.crawl()이 호출되지 않음을 확인합니다.
     *
     ...
     */
    @Property(tries = 100)
    void executeSingleShouldReturnNullWhenAlreadyRunning(
            @ForAll @IntRange(min = 0, max = 1000) final int seed) throws Exception {
        ...
        final CrawlerService crawlerService =
                new CrawlerService(mockCrawlerEngine, mockCrawlerConfig);
        ...
        final TriggerSource triggerSource =
                seed % 2 == 0 ? TriggerSource.SCHEDULED : TriggerSource.MANUAL;
    ...
    /**
     * N개 결과를 추가한 후 getRecentResults() 크기가 min(N, 20) 이하이며, 목록이 시간 역순(최신 우선)으로 정렬됨을 검증합니다.
     *
     * <p>리플렉션을 사용하여 private addToRecentResults 메서드를 호출하고, 각 결과에 1분씩 증가하는 고유 타임스탬프를 부여하여 정렬 순서를
     * 확인합니다.
     *
    ...
    void recentResultsShouldBeLimitedAndSortedInReverseChronologicalOrder(
        ...
        final Method addToRecentResultsMethod =
                CrawlerService.class.getDeclaredMethod("addToRecentResults", CrawlResult.class);
