 ...
 * 크롤러 관리 웹 UI 컨트롤러.
 *
 * <p>대시보드 화면을 제공하며, 수동 크롤링 실행 및 결과 조회, 스케줄러 상태 확인 기능을 담당합니다.
 */
@Controller
...
public class AdminController {
     ...
     *
     * <p>크롤링이 이미 실행 중이면 경고 메시지와 함께 대시보드로 리다이렉트합니다. 미등록 타겟인 경우 오류 메시지를, 정상 완료 시 성공 메시지를 flash 속성으로
     * 전달합니다.
     *
    ...
    public String triggerSingleCrawl(
            @PathVariable final String targetName, final RedirectAttributes redirectAttributes) {
        if (crawlerService.isRunning() || schedulerService.isScheduledRunning()) {
            redirectAttributes.addFlashAttribute(
                    "warningMessage", "크롤링이 이미 실행 중입니다. 완료 후 다시 시도해주세요.");
        ...
        if (result == null) {
            redirectAttributes.addFlashAttribute(
                    "errorMessage", "크롤링 대상을 찾을 수 없습니다: " + targetName);
            return "redirect:/admin";
        }

        redirectAttributes.addFlashAttribute("successMessage", targetName + " 크롤링이 완료되었습니다");
        return "redirect:/admin";
    }
    ...
    private void populateDashboardModel(final Model model) {
        ...
        model.addAttribute(
                "nextExecutionTime", schedulerService.getNextExecutionTime().orElse(null));
