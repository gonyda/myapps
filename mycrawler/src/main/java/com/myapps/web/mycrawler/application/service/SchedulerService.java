 ...
 * 크롤링 스케줄링을 관리하는 서비스.
 *
 * <p>{@link SchedulingConfigurer}를 구현하여 매 트리거 시점마다 cron 표현식을 재평가합니다. 이를 통해 애플리케이션 재시작 없이 설정 변경을 반영할
 * 수 있습니다.
 *
 * <p>시작 시점에 cron 표현식의 유효성을 검증하며, 유효하지 않은 경우 에러 로그를 출력하고 스케줄링을 비활성화한 채로 애플리케이션을 정상 기동합니다.
 */
@Service
public class SchedulerService implements SchedulingConfigurer {
     ...
     *
     * <p>생성 시점에 cron 표현식의 유효성을 검증하여 스케줄링 활성화 여부를 결정합니다.
     *
     ...
     * 스케줄링 태스크를 등록합니다.
     *
     * <p>매 트리거 시점마다 cron 표현식을 재평가하는 트리거 태스크를 등록합니다. 스케줄링이 비활성화된 경우 태스크를 등록하지 않습니다.
     *
     ...
     *
     * <p>현재 설정된 cron 표현식 기준으로 다음 실행 시각을 계산하여 "yyyy-MM-dd HH:mm:ss" 형식의 KST 문자열로 반환합니다. 스케줄링이
     * 비활성화되었거나 cron이 유효하지 않으면 빈 Optional을 반환합니다.
     *
    ...
    private void executeSingleTarget(final CrawlTarget target) {
        try {
            final CrawlResult result =
                    crawlerService.executeSingle(target.name(), TriggerSource.SCHEDULED);
            if (result == null) {
                log.error("스케줄 크롤링 결과가 null입니다. targetName={}", target.name());
            }
        } catch (final Exception exception) {
            log.error(
                    "스케줄 크롤링 중 예외 발생. targetName={}, error={}",
                    target.name(),
