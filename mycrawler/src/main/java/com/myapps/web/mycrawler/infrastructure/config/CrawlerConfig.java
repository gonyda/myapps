 ...
 * 크롤러 설정을 바인딩하는 레코드.
 *
 * <p>application.yml의 crawler 접두사 하위 속성을 매핑하며, 타겟 목록에 대한 유효성 검증 기능을 제공합니다.
 *
...
public record CrawlerConfig(
        String cron, long timeoutSeconds, String browsersPath, List<TargetConfig> targets) {
     ...
     *
     * <ul>
     *   <li>이름이 null이거나 빈 문자열인 경우
     *   <li>URL이 null이거나 빈 문자열인 경우
     *   <li>URL 형식이 유효하지 않은 경우
     *   <li>이름이 중복된 경우 (첫 번째 항목만 유지)
     * </ul>
     *
    ...
    private boolean isValidTarget(final TargetConfig target, final Set<String> seenNames) {
        ...
        if (!isValidUrl(target.url())) {
            log.warn(
                    "크롤링 대상의 URL 형식이 유효하지 않습니다. 해당 항목을 무시합니다. name={}, url={}",
                    target.name(),
                    target.url());
            return false;
        }
