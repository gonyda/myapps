 ...
 *
 * <p>Thymeleaf 템플릿에 일정, 기념일, D-Day 카운터 데이터를 전달하여 캘린더 뷰를 구성합니다. 루트 경로 접근 시 현재 월 캘린더로 리다이렉트합니다.
 */
@Controller
public class CalendarController {
     ...
     *
     * <p>해당 월의 일정, 기념일, D-Day 카운터, 이전/다음 월 네비게이션 정보를 모델에 추가하여 Thymeleaf 템플릿에 전달합니다.
     *
     * @param year 조회 대상 연도
    ...
    public String showCalendar(
        ...
        final List<Anniversary> anniversaries =
                anniversaryCalculator.getAnniversariesForMonth(yearMonth);
        ...
        final LocalDate weekStart =
                weekReferenceDate.with(TemporalAdjusters.previousOrSame(DayOfWeek.SUNDAY));
        final LocalDate weekEnd = weekStart.plusDays(6);
        final Map<LocalDate, List<ScheduleResponse>> weeklyScheduleMap =
                buildWeeklyScheduleMap(weekStart, weekEnd);
    ...
    public String weeklySchedule(
            @RequestParam("startDate") final LocalDate startDate, final Model model) {
        final LocalDate weekStart =
     ...
     * 주간 기준 날짜를 결정합니다.
     *
     * <p>현재 표시 중인 월에 오늘이 포함되면 오늘을 기준으로, 그렇지 않으면 해당 월의 1일을 기준으로 합니다.
     *
     * @param today 오늘 날짜
     ...
     */
    private Map<LocalDate, List<ScheduleResponse>> buildWeeklyScheduleMap(
        ...
        for (LocalDate date = weekStart; !date.isAfter(weekEnd); date = date.plusDays(1)) {
            ...
            final List<ScheduleResponse> daySchedules =
                    weekSchedules.stream().filter(s -> overlapsDate(s, currentDate)).toList();
