 ...
 *
 * <p>Thymeleaf 템플릿에서 SpEL로 직접 표현하기 어려운 일정 필터링 로직을 제공합니다. Model에 추가되어 템플릿에서 직접 호출됩니다.
 */
@Component
public class CalendarViewHelper {
     ...
     *
     * <p>일정이 해당 날짜와 겹치는 조건: startDate &lt;= date AND (endDate &gt;= date OR (endDate == null AND
     * startDate == date))
     *
     ...
     */
    public List<ScheduleResponse> getSchedulesForDate(
            final List<ScheduleResponse> schedules, final LocalDate date) {
        return schedules.stream().filter(s -> overlaps(s, date)).toList();
    }
