 ...
 *
 * <p>수정 전 코드에서 정상 동작하는 HTTP 요청/응답 패턴을 검증합니다. 일정 생성 POST 리다이렉트, 캘린더 조회 200, 일정 상세 조회 200을 확인하여 수정
 * 후에도 동일한 동작이 보존되는지 테스트합니다.
 *
...
class PreservationMockMvcTest {
    ...
    /**
     * POST /schedules 요청이 일정을 생성하고 캘린더 페이지로 302 리다이렉트하는지 확인합니다.
     *
    ...
    void should_redirect302ToCalendar_when_scheduleCreated() throws Exception {
        final ScheduleResponse response = createScheduleResponse(1L, LocalDate.of(2026, 7, 15));
    ...
    /**
     * POST /schedules 요청에서 endDate와 scheduleTime이 있는 경우에도 정상 302 리다이렉트하는지 확인합니다.
     *
     * <p><b>Validates: Requirements 3.1</b>
     */
    @Test
    void should_redirect302ToCalendar_when_scheduleCreatedWithEndDateAndTime() throws Exception {
        final ScheduleResponse response = createScheduleResponse(2L, LocalDate.of(2026, 3, 10));
        when(scheduleService.create(any())).thenReturn(response);
    ...
    /**
     * GET /calendar/{year}/{month} 요청이 200을 반환하고 모델에 올바른 prev/next 월 정보를 포함하는지 확인합니다.
     *
    ...
    /**
     * GET /calendar/{year}/{month} 요청에서 12월→1월 경계가 올바르게 처리되는지 확인합니다.
     *
    ...
    /**
     * GET /calendar/{year}/{month} 요청에서 1월→12월(전년) 경계가 올바르게 처리되는지 확인합니다.
     *
     * <p><b>Validates: Requirements 3.2, 3.6</b>
     */
    @Test
    ...
    /**
     * GET /schedules/{id} 요청이 200을 반환하고 일정 JSON 데이터를 정상적으로 반환하는지 확인합니다.
     *
    ...
    void should_return200WithScheduleData_when_detailRequested() throws Exception {
        ...
        mockMvc.perform(get("/schedules/1")).andExpect(status().isOk());
    }

    private ScheduleResponse createScheduleResponse(final Long id, final LocalDate startDate) {
