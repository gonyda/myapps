 ...
 * 버그 조건 탐색 테스트.
 *
 * <p>수정 전 코드에서 버그를 재현하여 근본 원인 분석을 확인합니다. 이 테스트들은 미수정 코드에서 실패(FAIL)하는 것이 정상이며, 실패는 버그의 존재를 증명합니다.
 *
...
class BugConditionExplorationTest {
    ...
    /**
     * Bug Condition 1: POST /schedules/{id} + _method=PUT 전송 시 HiddenHttpMethodFilter를
     * 통해 @PutMapping("/{id}")으로 정상 라우팅되어 302 리다이렉트가 반환되는지 확인합니다.
     *
     * <p>미수정 코드에서는 HiddenHttpMethodFilter가 비활성화되어 있으므로 _method=PUT이 무시되고 POST /schedules/1로 처리되어
     * 405가 반환됩니다. 따라서 이 테스트는 미수정 코드에서 실패합니다.
     *
    ...
    /**
     * Bug Condition 2: POST /schedules/{id} + _method=DELETE 전송 시 HiddenHttpMethodFilter를
     * 통해 @DeleteMapping("/{id}")으로 정상 라우팅되어 302 리다이렉트가 반환되는지 확인합니다.
     *
     * <p>미수정 코드에서는 HiddenHttpMethodFilter가 비활성화되어 있으므로 _method=DELETE가 무시되고 POST /schedules/1로 처리되어
     * 405가 반환됩니다. 따라서 이 테스트는 미수정 코드에서 실패합니다.
     *
    ...
    void should_routeToDeleteMapping_when_postWithMethodDelete() throws Exception {
        ...
        mockMvc.perform(post("/schedules/{id}", scheduleId).param("_method", "DELETE"))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/calendar/2026/7"));
    }

    /**
     * Bug Condition 3: CalendarController가 한국 시간(Asia/Seoul) 기준 정확한 오늘 날짜를 모델에 설정하는지 검증합니다.
     *
     * <p>CalendarController.showCalendar()가 {@code LocalDate.now(ZoneId.of("Asia/Seoul"))}을 사용하여
     * today 속성을 설정해야 합니다. JVM 기본 타임존에 관계없이 한국 시간 기준 정확한 오늘 날짜가 모델에 전달되어야 합니다.
     *
     * <p>이 테스트는 MockMvc를 통해 실제 CalendarController를 호출하고, 모델의 today 속성이 {@code
     * LocalDate.now(ZoneId.of("Asia/Seoul"))}과 일치하는지 확인합니다. 미수정 코드에서 LocalDate.now()가 ZoneId 없이
    ...
    void should_returnKoreanDate_when_calendarIsRendered() throws Exception {
        ...
        when(anniversaryCalculator.getAnniversariesForMonth(any()))
        ...
        final MvcResult result =
                mockMvc.perform(get("/calendar/2026/7")).andExpect(status().isOk()).andReturn();
