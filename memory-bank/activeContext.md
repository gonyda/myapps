...
- **맵 & 몬스터 데이터 조정 및 신규 추가 완료 (`docs/todo.md` 2번 섹션)**:
  - `거미` $\rightarrow$ `흰 거미` 명칭 변경 및 밸런스 하향 (CP 45.4, 쉬움)
  - `너구리` 밸런스 하향 (CP 37.5, 쉬움)
  - `붉은 여우` (`red-fox`, Lv2, CP 41.5, 쉬움) 스탯 조정 (너구리와 흰 거미 사이 위치) 및 `동쪽 언덕` 스폰 연동
  - `공동 묘지` (`graveyard`, x:1, y:-1) 신규 맵 노드 추가 및 `흰 거미` 스폰 연동
  - 드랍 테이블 개편: 너구리/붉은여우/흰거미 포션 3종(HP/MP/스태미나) 드랍, 던전 일반 몬스터(붉은거미/고블린/검은거미) 골드 전용 드랍화
  - 5대 품질 가드레일(Spotless, Error Prone, ArchUnit, JaCoCo, PMD/CPD) 및 1,045개 단위/통합 테스트 전수 통과
- **기획 백로그 (`docs/todo.md` 현황)**:
  ...
  4. **개발/테스트 도구**: 1,000 EXP / 1,000 Gold 획득 치트 버튼
