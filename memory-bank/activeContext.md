...
- **스킬 팝업 UI 레이아웃 개편 및 모바일 세로모드(360~480px) 최적화 완료**:
  - **2행 다크 판타지 카드 레이아웃**: 1행(아이콘/스킬명/랭크 & 쿨타임/사용버튼), 2행(풀너비 75% 수련치 게이지 & 승급버튼)으로 분리하여 게이지 찌그러짐 문제 완전 해결.
  - **탤런트 아이콘 매핑**: 근접(`⚔️`), 활(`🏹`), 마법(`🔮`), 공용(`🌟`) 계열별 직관적 아이콘 및 앤틱 골드/네온 시각 연출.
