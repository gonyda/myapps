...
- **스킬 시스템 정합성 및 다형성 확장 리팩토링 완료**:
  - **도메인 다형성(Effect-Driven Polymorphism)**: `SkillEffectRowView`, `SkillRankupBonusDelta` 신설 및 8종 스킬 도메인 모델(`DamageSkill`, `DefenseSkill`, `RecoverySkill`, `UltimateSkill`, `PassiveSkill`, `BuffSkill`, `CcSkill`, `DotSkill`)에 `effectRowsAt`, `rankupBonusDelta` 다형적 인터페이스 구현.
  - **스탯 및 수련 조건 정합성 해결**:
    - 메디테이션(패시브): F->E 승급 시 MP +2, MP_REGEN +0 정상 반영 (INT +1 오표기 및 하드코딩 제거).
    - 디펜스(방어): 승급 시 `방어력(DEF) +1, 최대 HP +5` 정상 표기, 막타 처치 수련 조건 면제(`hasKillRequirement=false`)로 UI에서 불필요한 막타 행 숨김 처리.
    - 패시브 스킬: 수련 방법 섹션에 "패시브 스킬은 수련 없이 AP로 즉시 승급할 수 있습니다" 앤틱 골드 안내 박스 렌더링.
  - **Python 밸런스 검증 도구 동기화**: `tools/balance/verify_skill.py` 8개 전 스킬 도메인(31종 스킬) 16키 및 단조성 검증 확장, 100% 무경고 통과.
  - **가드레일 검증**: 스킬 관련 175개 테스트 포함 전체 1,173개 테스트 100% 통과 (`BUILD SUCCESS`), PMD 인지 복잡도 0건, CodeGraph 동기화 완료.
