 ...
 *
 * <p>1개 이상의 {@code CharacterProgress}가 저장소에 존재하면 {@code loadOrCreateDefault()} 호출 시 새 캐릭터를 생성/저장하지
 * 않고 기존 진행상황을 그대로 반환해야 한다.
 *
...
class CharacterServiceLoadExistingPropertyTest {
    ...
    /**
     * 저장소에 1개 이상의 CharacterProgress가 존재할 때 loadOrCreateDefault()는 기존 진행상황을 반환하고 save()를 호출하지 않는다.
     *
    ...
    void should_returnExistingProgress_when_storeHasCharacter(
        ...
        final CharacterProgress existingProgress =
        ...
        verify(mockRepository, never())
                .save(org.mockito.ArgumentMatchers.any(CharacterProgress.class));
