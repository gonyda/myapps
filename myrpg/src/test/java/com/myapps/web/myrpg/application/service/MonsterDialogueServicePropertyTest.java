 ...
 *
 * <p>{@link MonsterDialogueService#selectLine(Monster)}이 항상 해당 몬스터의
 * {@code lines}에 포함된 값을 반환하고, 고정 시드 {@code Random}에서 결정적이며,
 * 폴백 문구를 사용하지 않음을 검증한다.
 *
...
class MonsterDialogueServicePropertyTest {
    ...
    /**
     * 임의의 몬스터에 대해 {@code selectLine}의 반환값은 항상
     * 해당 몬스터의 {@code lines} 목록에 포함된다.
     *
    ...
    /**
     * 동일 시드의 {@code Random}으로 생성한 두 서비스 인스턴스가 동일한 몬스터에 대해
     * 동일한 대사 선택 시퀀스를 반환함을 검증한다(결정적).
     *
     * @param monster 임의 생성된 몬스터 (lines 3개 보유)
     */
    @Property(tries = 100)
    void should_produceDeterministicResults_when_sameSeedUsed(
        ...
        for (int i = 0; i < DETERMINISM_ITERATIONS; i++) {
            ...
            assertThat(result1)
                    .as("호출 %d번째에서 동일 시드는 동일 결과를 반환해야 합니다", i)
                    .isEqualTo(result2);
        }
    }

    /**
     * {@code selectLine}의 반환값은 폴백 문구가 아니라 반드시 몬스터의 원래
     * {@code lines} 3개 중 하나와 정확히 일치한다.
     *
    ...
    void should_neverUseFallbackText_when_linesAlwaysHaveThreeEntries(
        ...
        for (int i = 0; i < DETERMINISM_ITERATIONS; i++) {
            ...
            assertThat(result).isIn(originalLines.get(0), originalLines.get(1), originalLines.get(2));
        }
    }
     ...
     *
     * <p>각 몬스터는 정확히 3개의 비어있지 않은 고유 대사를 보유한다.
     * jqwik {@code Combinators.combine}은 최대 8인자까지 지원하므로
     * 두 단계로 나누어 결합한다.
     *
    ...
    Arbitrary<Monster> validMonsters() {
        ...
        final Arbitrary<GoldDrop> goldDrops = Arbitraries.integers().between(0, 50)
