 ...
 *
 * <p>임의 몬스터 목록과 맵 노드 배치에 대해 {@code byNode(nodeId)}가
 * 맵 정의 순서를 보존하며, 미지 노드·null에는 빈 목록을 반환함을 검증한다.
 *
...
class MonsterServiceByNodePropertyTest {
    ...
    void should_preserveMapOrder_when_byNodeCalled(
        ...
        final List<String> resultIds = result.stream()
                .map(Monster::id)
                .toList();
    ...
    Arbitrary<List<MonsterInputData>> validMonsterDataset() {
        return Arbitraries.integers().between(2, MAX_MONSTER_COUNT)
    ...
    Arbitrary<List<Integer>> nodeMonsterIndices() {
        return Arbitraries.integers().between(0, MAX_MONSTER_COUNT - 1)
                .list().ofMinSize(1).ofMaxSize(MAX_MONSTER_COUNT);
    }
    ...
    private List<MonsterInputData> ensureUniqueIds(final List<MonsterInputData> rawList) {
        ...
        for (int i = 0; i < rawList.size(); i++) {
            ...
            result.add(new MonsterInputData(
                    ...
                    original.goldDropMin(), original.goldDropMax(), original.lines()));
        }
        ...
        return Combinators.combine(ids, names, types, statGroup, lines)
                .as((id, name, type, stats, linesList) ->
                        new MonsterInputData(id, name, type,
                                ...
                                stats.goldDrop()[0], stats.goldDrop()[1], linesList));
    }
    ...
    private MonsterService buildServiceWithNodeMapping(
            ...
            final List<String> nodeMonsterIds) {
        ...
        final MapNode targetNode = new MapNode(
                TARGET_NODE_ID, "Test Node", "field", null,
                0, 0, null, null, List.of(), nodeMonsterIds);
        ...
        final MonsterService monsterService = new MonsterService(
                objectMapper, mapService, itemCatalogService);

        final List<Monster> loaded = monsterService.loadFromStream(inputStream);
        setField(monsterService, "monsters", loaded);

        final Map<String, Monster> byIdMap = loaded.stream()
                .collect(Collectors.toUnmodifiableMap(Monster::id, m -> m));
    ...
    private void setField(final Object target, final String fieldName, final Object value) {
        try {
            ...
            throw new RuntimeException(
                    "MonsterService." + fieldName + " 필드 접근 실패", exception);
        }
    }
