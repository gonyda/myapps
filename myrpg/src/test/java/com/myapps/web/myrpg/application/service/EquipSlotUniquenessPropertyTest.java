 ...
 *
 * <p>임의의 착용/해제 시퀀스 후에도 각 {@link EquipSlot}을 점유한 장착 장비는 최대 1개이며(양손검은 MAIN_HAND+OFF_HAND를 동시 점유), 장착
 * 장비는 모두 {@code storage=INVENTORY}임을 검증한다.
 *
...
class EquipSlotUniquenessPropertyTest {
    ...
    /**
     * 임의의 equip/unequip 시퀀스 실행 후 각 슬롯을 점유한 장착 장비가 최대 1개이고 장착 장비가 모두 INVENTORY임을 검증한다.
     *
    ...
    void should_maintainSlotUniqueness_after_equipUnequipSequence(
        ...
        final EquipmentItem oneHandItem =
                createEquipmentItem(
        ...
        for (final int op : operations) {
            ...
            final List<OwnedItem> currentEquipped =
                    allItems.stream().filter(OwnedItem::isEquipped).toList();
        ...
        final List<OwnedItem> finalEquipped =
                allItems.stream().filter(OwnedItem::isEquipped).toList();
    ...
    void should_haveUniqueSlotOccupancy_when_defaultSeedEquipped(
        ...
        final Map<String, EquipmentKind> kindMap =
                Map.of(
        ...
        for (final EquipSlot slot : EquipSlot.values()) {
            if (defaultOccupiedSlots.contains(slot)) {
                assertThat(slotCount.get(slot)).as("기본 장착 시 슬롯 %s는 정확히 1개 점유", slot).isEqualTo(1);
            } else {
                assertThat(slotCount.get(slot)).as("기본 장착에 쓰이지 않는 슬롯 %s는 0 점유", slot).isZero();
            }
        }
    ...
    /**
     * 양손검 장착 시 MAIN_HAND + OFF_HAND 두 슬롯을 동시 점유하여 다른 장비가 두 슬롯에 추가 장착 불가임을 검증한다.
     *
     * @param ignored 프로퍼티 실행을 100회 반복하기 위한 임의 값
     */
    @Property(tries = 100)
    void should_occupyBothSlots_when_twoHandedSwordEquipped(
        ...
        when(mockRepo.findByStorageAndEquippedTrue(StorageKind.INVENTORY)).thenReturn(List.of());
    ...
    /**
     * 조작 시퀀스를 생성하는 Arbitrary 제공자. 양수: equip, 음수: unequip. 절대값 % 4로 아이템 인덱스 결정.
     *
     * @return 정수 리스트의 Arbitrary (3~10개 조작)
     */
    @Provide
    Arbitrary<List<Integer>> operationSequence() {
        return Arbitraries.integers().between(-100, 100).list().ofMinSize(3).ofMaxSize(10);
    }
