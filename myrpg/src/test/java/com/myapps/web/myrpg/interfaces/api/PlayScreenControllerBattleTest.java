 ...
 *
 * <p>기습 발동 시 자동 전투 시작 및 {@code ambushMonsterName} 설정,
 * 기습 미발동 시 신호 미포함, {@code GET /} 전투 재개 시 {@code battleView}·{@code battleActive} 설정,
 * 전투 중 {@code POST /move} 이동 거부를 검증한다.
 *
...
class PlayScreenControllerBattleTest {
    ...
    /**
     * POST /move 이동 성공 시 기습이 발동하면 {@code ambushMonsterName}이 모델에 설정되고
     * {@code battleService.start}가 {@code ambush=true}로 호출되는지 검증한다.
     *
    ...
    void should_setAmbushSignalAndStartBattle_when_preemptiveStrikeTriggered() throws Exception {
        ...
        final List<InteractionItem> interactions = List.of(
                new InteractionItem("raccoon", "너구리", false)
        );
        ...
        when(monsterEncounterService.rollPreemptiveStrike(List.of(raccoon))).thenReturn(Optional.of(raccoon));
        when(battleService.start(any(CharacterProgress.class), eq("raccoon"), eq(true))).thenReturn(battleState);
    ...
    /**
     * POST /move 이동 성공 시 기습이 발동하지 않으면 {@code ambushMonsterName}이 모델에 없고
     * {@code battleService.start}가 호출되지 않는지 검증한다.
     *
     * <p><b>Validates: Requirements 17.1, 17.3</b>
     */
    @Test
    void should_notSetAmbushSignal_when_preemptiveStrikeNotTriggered() throws Exception {
        ...
        when(monsterService.byNode(anyString())).thenReturn(List.of(raccoon));
        when(playScreenViewHelper.buildInteractions(anyList(), anyList())).thenReturn(interactions);
        when(monsterEncounterService.rollPreemptiveStrike(List.of(raccoon))).thenReturn(Optional.empty());
        when(progressionService.rebirthStatus(any(CharacterProgress.class)))
                .thenReturn(new RebirthStatus(true, false, null, null));
        when(playScreenViewHelper.buildInfo(any(CharacterProgress.class), any(RebirthStatus.class)))
                .thenReturn(dummyInfo());
    ...
    /**
     * GET / 요청 시 활성 전투가 존재하면 모델에 {@code battleView}와
     * {@code battleActive=true}가 포함되어 전투가 복원되는지 검증한다.
     *
    ...
    void should_restoreBattleView_when_activeBattleExistsOnPageLoad() throws Exception {
        ...
        final List<BattleSkillButton> skills = List.of(
        ...
        when(characterService.loadOrCreateDefault()).thenReturn(progress);
        when(battleService.resumeIfActive(any(CharacterProgress.class))).thenReturn(Optional.of(battleState));
        ...
        final BattleView actualBattleView = (BattleView) result.getModelAndView().getModel().get("battleView");
    ...
    /**
     * GET / 요청 시 저장된 monsterId가 카탈로그에서 소실되었으면
     * 전투가 안전 종료되어 {@code battleView}가 모델에 없는지 검증한다.
     *
    ...
    /**
     * POST /move 요청 시 활성 전투가 존재하면 이동이 거부되고
     * 실제 이동(movementService.move)이 호출되지 않는지 검증한다.
     *
    ...
    private Monster createRaccoon() {
        return new Monster(
                "raccoon", "너구리", MonsterType.NORMAL,
                1, 25, 4, 1, 10, 15L,
                new GoldDrop(3, 10), List.of(), List.of(
                "크르릉… 쉭, 쉭!",
    ...
    private InfoPopupView dummyInfo() {
        ...
        return new InfoPopupView(
                "고니", 1, 1, "근접전투", 0, "근접 데미지 +10%, STR +2/Lv, HP +5/Lv",
                gauge, gauge, gauge, List.of(), true, "환생 기록 없음"
        );
    }
