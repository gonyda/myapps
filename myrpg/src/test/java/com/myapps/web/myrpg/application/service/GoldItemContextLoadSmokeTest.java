...
class GoldItemContextLoadSmokeTest {
    ...
    GoldItemContextLoadSmokeTest(final ItemCatalogService itemCatalogService,
     ...
     *
     * <p>{@link InventoryService#equippedBonus()}가 유효한 결과를 반환하고
     * statBonus/vitalBonus가 모두 null이 아닌지 확인한다.
     */
    @Test
     ...
     *
     * <p>설계에 따르면 골드 표시는 인벤토리·은행 팝업에서만 수행되며
     * 상단바는 변경되지 않는다(Requirements 20.5). 이 테스트는
     * top-bar 관련 뷰 모델에 골드 필드가 존재하지 않음을 구조적으로 검증한다.
     *
     * <p>TopBarView가 gold 필드를 갖지 않는 것으로 설계되었으므로
     * 컴파일 시점에서 보장되지만, 런타임 확인으로 문서화한다.
     */
    @Test
    void should_notExposeGoldInTopBar() {
        // TopBarView record에 gold() 메서드가 존재하지 않음을 리플렉션으로 확인.
        // gold 메서드가 없다면 상단바에 골드가 노출될 수 없다.
        final boolean hasGoldMethod = java.util.Arrays.stream(
                        com.myapps.web.myrpg.application.dto.TopBarView.class.getDeclaredMethods())
                .anyMatch(method -> "gold".equals(method.getName()));

        assertThat(hasGoldMethod)
                .as("TopBarView에 gold() 메서드가 없어야 한다 (상단바에 골드 미표시)")
                .isFalse();
    }
}
