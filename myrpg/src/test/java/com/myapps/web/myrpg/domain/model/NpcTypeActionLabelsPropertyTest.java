 ...
 *
 * <p>모든 {@link NpcType} 상수에 대해 {@code actionLabels}는 null이 아니며 비어 있지 않은 불변 목록이고,
 * 특히 {@code MAGIC_SCHOOL}은 {@code ["상점", "인챈트"]},
 * {@code BLACKSMITH}는 {@code ["상점", "수리"]},
 * {@code HEALER}는 {@code ["상점", "치료받기"]}를 정확히 포함함을 검증한다.
 *
...
class NpcTypeActionLabelsPropertyTest {
    ...
    /**
     * 모든 NpcType에 대해 actionLabels()는 null이 아니며 비어 있지 않고,
     * 타입별 정의된 행동 라벨 계약을 만족함을 검증한다.
     *
     * @param npcType 임의의 NpcType 상수
     */
    @Property(tries = 100)
    void should_satisfyActionLabelsContract_when_anyNpcType(@ForAll("npcTypes") final NpcType npcType) {
