 ...
 *
 * <p>임의의 유효 맵 그래프에서 그래프에 존재하지 않는 현재 노드 ID로 {@link MapViewFactory#createMinimap} 또는 {@link
 * MapViewFactory#createFullMap}을 호출하면 {@link MapViewGenerationException}이 발생하고, 어떤 셀이나 간선도 생성되지
 * 않음(부분 결과 없음)을 검증한다.
 *
...
class MapViewFactoryFailurePropertyTest {
    ...
    /**
     * 그래프에 존재하지 않는 현재 노드 ID로 미니맵 생성 시 {@link MapViewGenerationException}이 발생하고 부분 결과가 생성되지 않음을
     * 검증한다.
     *
    ...
    void should_throwMapViewGenerationException_when_minimapWithUnknownNodeId(
            @ForAll("validGraphWithUnknownNodeId")
                    final Tuple.Tuple2<MapGraph, String> graphAndUnknownId) {
    ...
    /**
     * 그래프에 존재하지 않는 현재 노드 ID로 전체지도 생성 시 {@link MapViewGenerationException}이 발생하고 부분 결과가 생성되지 않음을
     * 검증한다.
     *
     * @param graphAndUnknownId 임의 생성된 맵 그래프와 그래프에 존재하지 않는 노드 ID 튜플
     */
     ...
     *
     * <p>UUID 기반으로 그래프에 절대 존재하지 않는 노드 ID를 생성하여 "미확인 현재 노드" 시나리오를 확실히 재현한다.
     *
    ...
    private Arbitrary<Tuple.Tuple2<MapGraph, String>> buildGraphWithUnknownNodeId(
            final int gridSize) {
