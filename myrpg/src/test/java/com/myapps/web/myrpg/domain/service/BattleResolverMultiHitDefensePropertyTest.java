 ...
 *
 * <p>방어 > 0인 대상에 대해, 각 히트의 기본피해는 {@code max(1, floor(공격력×per-hit배율/100) − 방어)}로 방어가 히트마다 차감되며, 같은 총
 * 배율을 단일로 때린 경우보다 다단 총 피해가 크지 않다(고방어일수록 다단 불리).
 *
...
class BattleResolverMultiHitDefensePropertyTest {
     ...
     *
     * <p>편차에 의한 차이를 배제하기 위해 baseDamage 레벨에서 비교한다. 다단은 히트마다 방어를 빼므로 방어가 높을수록 불리하다.
     *
     * <p>비교가 유효한 조건: 1히트당 원시 피해(floor(atk×perHitMult/100)−defense)가 양수여야 함. min-floor(max(1,...))가
     * 개입하면 다단 합이 단일을 초과할 수 있으므로(min 보장 × hitCount), 1히트당 원시가 양수인 경우에만 비교한다.
     *
    ...
    void should_multiHitBaseDamageSumNotExceedSingleHitBaseDamage_when_defensePositive(
        ...
        final int multiBaseDmgSum =
                hitCount * resolver.baseDamage(attackPower, perHitMultiplier, defense);
    ...
    void should_eachHitBaseDamageMatchFormula_when_anyInput(
        ...
        final int expected =
                Math.max(1, Math.floorDiv(attackPower * perHitMultiplier, 100) - defense);
