 ...
 *
 * <p>임의의 재능 T에 대해, 레벨 1에서 {@code levelStatsFor(1, T)}는 공통 기본값과 같고
 * {@code vitalMaxFor(1, T)}는 세 바이탈 모두 100인지 검증한다. 이는 재능 보너스
 * 공식이 {@code perLevel × (level - 1)}이므로 level=1이면 보너스가 0이 되어
 * 환생(레벨 1 복귀) 시 자동 초기화가 보장됨을 확인한다.
 *
...
class RebirthTalentResetPropertyTest {
     ...
     *
     * <p>{@code perLevel × (1 - 1) = 0}이므로 재능 보너스가 가산되지 않아
     * 재능과 무관하게 공통 기본값만 반환되어야 한다.
     *
     ...
     *
     * <p>공통 바이탈 = {@code 100 + 10×(1-1) = 100}이고, 재능 보너스 = {@code perLevel × 0 = 0}이므로
     * 세 바이탈 모두 100이어야 한다.
     *
     * @param talent 임의의 재능 타입 (3종)
     */
