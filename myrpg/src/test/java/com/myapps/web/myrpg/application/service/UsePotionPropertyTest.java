 ...
 *
 * <p>임의의 HP 상태와 포션에 대해, {@code usePotion}은
 * {@code hpCurrent = min(hpCurrent + healHp, hpMax)}로 HP를 회복하고
 * {@code quantity}를 1 감소시키며, 0이 되면 행을 제거한다.
 * HP는 hpMax를 초과하지 않는다.
 *
...
class UsePotionPropertyTest {
    ...
    void should_clampHpToMax_when_potionUsed(
        ...
        final PotionItem potionItem = new PotionItem(
                params.potionId(), "테스트 포션", params.healHp(), 30);
     ...
     */
    @Property(tries = 100)
    void should_neverExceedHpMax_when_alreadyAtMax(
            @ForAll("healHpAmount") final int healHp) {
        ...
        final OwnedItem potionOwned = new OwnedItem(
                potionId, 5, StorageKind.INVENTORY, false, 0);
    ...
    /**
     * 지정된 HP 현재값을 가진 캐릭터를 생성한다.
     * 레벨 1, 재능 MELEE → vitalMaxFor(1, MELEE).hp() = 100 (장비 보너스 없음).
     *
