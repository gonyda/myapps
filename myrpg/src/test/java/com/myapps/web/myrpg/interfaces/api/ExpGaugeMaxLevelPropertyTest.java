 ...
 *
 * <p>레벨이 100 미만이면 percent와 overlay가 경험치/필요치로 계산되고, 레벨이 100이면 percent=100, overlay="MAX"임을 검증한다.
 *
...
class ExpGaugeMaxLevelPropertyTest {
    ...
    /**
     * 레벨이 100 미만일 때 EXP 게이지의 percent와 overlay가 경험치/필요치 기반으로 정확히 계산됨을 검증한다.
     *
    ...
    void should_calculateExpGauge_when_levelBelowMax(
        ...
        final int expectedPercent =
                (int)
                        Math.max(
                                0,
                                Math.min(
                                        100,
                                        Math.round((double) effectiveExp * 100 / requiredExp)));
