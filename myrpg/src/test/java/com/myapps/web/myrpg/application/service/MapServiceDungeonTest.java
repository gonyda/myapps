 ...
 *
 * <p>실제 {@code classpath:data/map.json}을 로드한 {@link MapService}로부터 던전 목록을 조회하여 다음을 검증한다:
 *
 * <ul>
 *   <li>모든 던전의 {@code implemented}가 {@code false}인지
 *   <li>모든 던전의 {@code map}이 {@code null}인지
 *   <li>{@code entranceNodeId}가 실제 노드 그래프에서 {@code type="dungeon"} 노드를 참조하는지
 *   <li>던전 입구 노드의 {@code dungeonId}가 해당 던전의 {@code id}와 일치하는지
 * </ul>
 *
...
class MapServiceDungeonTest {
    ...
    void should_haveImplementedFalse_when_allDungeonsExposed() {
        ...
        // Then
        assertThat(dungeons).allSatisfy(dungeon -> assertThat(dungeon.implemented()).isFalse());
    }
    ...
    void should_haveMapNull_when_allDungeonsExposed() {
        ...
        // Then
        assertThat(dungeons).allSatisfy(dungeon -> assertThat(dungeon.map()).isNull());
    }

    /** 각 던전의 {@code entranceNodeId}가 노드 그래프에 존재하며 해당 노드의 타입이 {@code dungeon}인지 검증한다. */
    @Test
    void should_referenceExistingDungeonNode_when_entranceNodeIdResolved() {
        ...
        for (final Dungeon dungeon : dungeons) {
            ...
            assertThat(entranceNode)
                    .as(
                            "던전 '%s'의 entranceNodeId '%s'가 노드 그래프에 존재해야 함",
    ...
    /**
     * 각 던전 입구 노드의 {@code dungeonId}가 해당 던전의 {@code id}와 일치하는지 검증한다. (던전 → 입구 노드 → dungeonId → 던전 id
     * 양방향 참조 무결성)
     */
    @Test
    void should_matchDungeonId_when_entranceNodeDungeonIdResolved() {
        ...
        for (final Dungeon dungeon : dungeons) {
            ...
            assertThat(entranceNode.dungeonId())
                    .as(
                            "입구 노드 '%s'의 dungeonId가 던전 '%s'의 id와 일치해야 함",
                            entranceNode.id(), dungeon.id())
                    .isEqualTo(dungeon.id());
        }
    }

    /** 모든 {@code type="dungeon"} 노드가 던전 정의에 의해 참조되는지 검증한다. (고아 던전 입구 노드가 없음을 보장) */
    @Test
    void should_coverAllDungeonNodes_when_dungeonEntranceReferencesChecked() {
        ...
        final Map<String, String> entranceNodeIdToDungeonId =
                dungeons.stream()
                        .collect(
                                Collectors.toUnmodifiableMap(Dungeon::entranceNodeId, Dungeon::id));

        final List<MapNode> dungeonTypeNodes =
                graph.nodes().stream().filter(node -> NodeType.DUNGEON == node.nodeType()).toList();
