...
class SkillRankupBonusTest {
    ...
    void should_addStrBonus_when_meleeSkillAtRankA() {
        ...
        final Function<String, Optional<Skill>> lookup = createLookup(
                "windmill", SkillTalent.MELEE);
    ...
    @Test
    @DisplayName("7종 전부 MASTER(order 15): MELEE×2+ARCHERY×2+MAGIC×2+COMMON×1 → STR+30/DEX+30/INT+30/DEF+15")
    void should_sumAllBonuses_when_allSkillsAtMaster() {
        ...
        final Function<String, Optional<Skill>> lookup = id -> {
            final SkillTalent talent = switch (id) {
    ...
    private Function<String, Optional<Skill>> createLookup(final String skillId,
