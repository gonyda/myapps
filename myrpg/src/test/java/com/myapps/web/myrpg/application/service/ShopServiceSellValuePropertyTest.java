 ...
 *
 * <p>아이템이 {@code buyPrice}를 가지면 {@code round(buyPrice * 0.5)}로 계산되고, buyPrice가 없으면 장비 보너스 합산({@code
 * Σ amount * weight})으로 계산되며, 두 경로가 동시에 적용되지 않는다(배타성)를 검증한다. 또한 같은 입력에 대해 결과가 항상 동일함(결정성)을 검증한다.
 *
 * <p>Feature: 010-npc-actions-shop-repair-heal, Property 1: 판매가 계산식 배타성 및 결정성 (BuyPrice vs Catalog
 * Bonuses)
 *
 * <p><b>Validates: Requirements 1.1, 1.2, 1.3, 1.4, 1.5, 1.8, 15.2</b>
 */
// Feature: 010-npc-actions-shop-repair-heal, Property 1: 판매가 계산식 배타성 및 결정성 (BuyPrice vs Catalog
// Bonuses)
class ShopServiceSellValuePropertyTest {
    ...
    /**
     * buyPrice가 존재하는 임의 아이템의 판매가는 항상 {@code round(buyPrice * 0.5)}와 일치하고, 보너스 합산이 아닌 단일 경로로
     * 계산됨(배타성)을 검증한다.
     *
    ...
    /**
     * buyPrice가 없는 장비 아이템의 판매가는 보너스 합산식 {@code Σ amount * weightOf(target)}과 일치하고, buyPrice 경로와
     * 배타적임을 검증한다.
     *
     * @param bonuses 임의 보너스 목록 (amount 1~100)
     */
    @Property(tries = 100)
    void should_sumCatalogBonuses_when_noBuyPrice(
            @ForAll("equipmentBonusList") final List<EquipBonus> bonuses) {
        final ShopService service = newService(fixedAction());
        final EquipmentItem equip =
                new EquipmentItem(
                        "armor",
        ...
        final long expected =
                bonuses.stream()
                        .mapToLong(
                                bonus -> (long) bonus.amount() * service.weightOf(bonus.target()))
                        .sum();
     ...
     */
    @Property(tries = 100)
    void should_returnZero_whenNoBuyPriceAndNoBonus(@ForAll("zeroScoringItem") final Item item) {
        final ShopService service = newService(fixedAction());
