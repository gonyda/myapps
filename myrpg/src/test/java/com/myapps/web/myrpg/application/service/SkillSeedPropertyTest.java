 ...
 * 기본 스킬 4종(베기·조준 사격·마나 볼트·디펜스)이 각각 F 랭크·usageCount 0·killCount 0으로
 * 정확히 한 번씩 저장되는지 검증한다.
 *
...
class SkillSeedPropertyTest {
    ...
    /**
     * 임의의 캐릭터 ID에 대해 seedDefault 호출 시, 기본 스킬 4종이
     * F 랭크·카운트 0으로 정확히 한 번씩 저장된다.
     *
    ...
    void should_seedFourDefaultSkills_when_seedDefaultCalled(
        ...
        assertThat(saved).allSatisfy(skill -> {
    ...
    private void stubCatalogAndRepository(final SkillCatalogService mockCatalog,
                                          final CharacterSkillRepository mockRepository,
                                          final Long characterId) {
        for (final String skillId : DEFAULT_SEED_SKILL_IDS) {
            final Skill catalogSkill = "defense".equals(skillId)
                    ? mock(DefenseSkill.class)
                    : mock(DamageSkill.class);
