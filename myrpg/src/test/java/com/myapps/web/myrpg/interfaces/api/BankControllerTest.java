 ...
 *
 * <p>은행 팝업 렌더, 골드 입출금 성공·부족 안내, 아이템 맡기기(장착 거부·용량 초과)·찾기를 검증한다.
 */
...
class BankControllerTest {
    ...
    @MockitoBean private BankService bankService;
    ...
    void should_returnBankPopupFragment_when_bankRequested() throws Exception {
        ...
        when(inventoryService.buildBankView(progress.getGold(), bank.getGold()))
                .thenReturn(bankView);
    ...
    void should_returnErrorView_when_depositExceedsPlayerGold() throws Exception {
        ...
        doThrow(new InsufficientGoldException("소지금 부족: 입금 요청 1000, 보유 0"))
                .when(bankService)
                .deposit(any(CharacterProgress.class), eq(1000L));
    ...
    void should_returnErrorView_when_withdrawExceedsBankGold() throws Exception {
        ...
        doThrow(new InsufficientGoldException("은행 잔액 부족: 출금 요청 1000, 보관 0"))
                .when(bankService)
                .withdraw(any(CharacterProgress.class), eq(1000L));
    ...
    private BankView dummyBankView() {
        final OwnedItemView potionView =
                new OwnedItemView(
                        1L,
                        "생명력 50 포션",
                        "포션",
        ...
        final OwnedItemView weaponView =
                new OwnedItemView(
                        2L,
                        "초보자 한손검",
                        "무기",
