 ...
 *
 * <p>임의의 보유 아이템 목록에 대해, 수리 목록({@link RepairView#repairItems()})에
 * 포함되는 아이템은 오직 {@link EquipmentItem}이면서
 * {@code ceil(currentDurability) < maxDurability}인 아이템뿐이고,
 * 포션 및 풀내구도 장비({@code ceil >= max})는 항상 제외됨을 검증한다.
 *
...
class RepairListFilterPropertyTest {
    ...
    /**
     * 임의의 보유 아이템 목록(0~8개)에 대해 수리 팝업 필터링 결과가
     * {@code EquipmentItem && ceil(current) < max} 조건과 정확히 일치하는지 검증한다.
     *
    ...
    void should_includeOnlyRepairableEquipment(@ForAll("ownedSeeds") final List<OwnedSeed> seeds) {
        ...
        final List<String> actualNames = view.repairItems().stream()
                .map(RepairItemView::name)
                .toList();

        final List<String> expectedNames = seeds.stream()
                .filter(seed -> seed.item() instanceof EquipmentItem equip
                        && Math.ceil(seed.durability()) < equip.maxDurability())
    ...
    Arbitrary<OwnedSeed> ownedSeed() {
        final Arbitrary<Item> equipment = Arbitraries.integers().between(1, 100)
                .map(maxDurability -> new EquipmentItem(
                        "eq-" + maxDurability, "장비" + maxDurability, ItemType.WEAPON,
                        EquipmentKind.ONE_HANDED_SWORD, List.of(), null, maxDurability));
        final Arbitrary<Item> potion = Arbitraries.integers().between(1, 100)
                .map(healHp -> new PotionItem("pot-" + healHp, "포션" + healHp, healHp, null));
        return Arbitraries.oneOf(equipment, potion)
                .flatMap(item -> Arbitraries.doubles().between(0.0, 200.0)
    ...
    private Fixture newFixture(final List<OwnedSeed> seeds) {
        ...
        when(catalogService.byId(anyString())).thenAnswer(invocation -> {
