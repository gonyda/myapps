 ...
 * 누적레벨 불변식 프로퍼티 테스트.
 *
 * <p>레벨업과 환생의 임의 시퀀스에서 {@code accumulatedLevel}이
 * 레벨업 1회당 +1, 환생 1회당 +1 증가하며 절대 감소하지 않음을 검증한다.
 *
...
class AccumulatedLevelInvariantPropertyTest {
    ...
    /**
     * 레벨업·환생 임의 시퀀스에서 누적레벨이 레벨업당 +1, 환생당 +1 증가하며
     * 감소하지 않음을 검증한다.
     *
    ...
    void should_maintainAccumulatedLevelInvariant_when_levelUpAndRebirthSequence(
        ...
        final Clock fixedClock = Clock.fixed(Instant.parse("2099-01-01T00:00:00Z"), ZoneId.of("UTC"));
        final ExperiencePolicy experiencePolicy = new ExperiencePolicy();
        final StatProgression statProgression = new StatProgression();
        final ProgressionService service = new ProgressionService(experiencePolicy, statProgression, fixedClock);
    ...
    Arbitrary<List<Action>> actionSequence() {
        final Arbitrary<Action> gainExp = Arbitraries.longs()
                .between(1L, 50000L)
                .map(Action.GainExp::new);

        final Arbitrary<Action> rebirth = Arbitraries.just(Action.Rebirth.INSTANCE);

        final Arbitrary<Action> action = Arbitraries.frequencyOf(
                net.jqwik.api.Tuple.of(7, gainExp),
                net.jqwik.api.Tuple.of(3, rebirth)
        );
