 ...
 *
 * <p>몬스터 피해·방어 반격 피해는 {@code hitCount}와 무관하게 단일 값으로 산출되고, 해당 경로(반격/0/교착)의 {@code playerHits}는 비어
 * 있다.
 *
...
class BattleResolverMonsterSingleHitPropertyTest {
    ...
    void should_playerHitsEmpty_when_attackLoses(
            @ForAll("seeds") final long seed, @ForAll("hitCounts") final int hitCount) {
     ...
     *
     * <p>동일 시드·동일 입력에서 hitCount만 달리한 두 결과의 몬스터 피해가 동일한지 확인한다. 공격 상성 패배(강&lt;일반) 경로에서 몬스터 피해만 산출되며
     * playerHitCount는 무관하다.
     *
     * @param seed 난수 시드
     */
    @Property(tries = 100)
    void should_monsterDamageUnchanged_when_hitCountVaries(@ForAll("seeds") final long seed) {

        final BattleResolver resolver1 = new BattleResolver(new Random(seed));
        final TurnInput input1 =
                new TurnInput(
                        SkillType.HEAVY,
                        SkillType.NORMAL,
                        DEFAULT_ATTACK_POWER,
