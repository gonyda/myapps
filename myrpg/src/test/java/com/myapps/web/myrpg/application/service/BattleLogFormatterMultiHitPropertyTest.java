 ...
 *
 * <p>임의의 {@code playerHits}에 대해, size ≥ 2면 헤더({@code "{스킬}({타입}) {N}연타"})와
 * 브레이크다운(각 히트 피해 나열, 크리 히트에 {@code "(치명)"}, 끝에 {@code "= {합계} 피해"})을
 * 생성하고, size ≤ 1이면 기존 단일 형식({@code "{스킬}({타입})로 … {N} 피해"}
 * (+{@code " (크리티컬!)"}))을 생성한다.
 *
...
class BattleLogFormatterMultiHitPropertyTest {
    ...
    /**
     * playerHits.size() ≥ 2일 때, 플레이어 로그가 헤더+브레이크다운 2줄로 구성되고
     * 헤더에 "{스킬}({타입}) {N}연타" 형식이며 브레이크다운에 각 히트 피해가
     * 올바른 순서·접미사로 나열되고 합계와 일치하는지 검증한다.
     *
    ...
    void should_produceHeaderAndBreakdown_when_multiHit(
        ...
        final BattleLogInput input = new BattleLogInput(
                SKILL_LABEL, skillType, MONSTER_NAME, SkillType.NORMAL,
                totalDamage, monsterDamage, false, false, false, hits);
        ...
        // 헤더 검증: "{스킬}({타입}) {N}연타"
        final String expectedHeader = SKILL_LABEL + "(" + skillType.label() + ") " + hits.size() + "연타";
    ...
    /**
     * playerHits.size() ≥ 2이고 firstStrike일 때, 선제 사격 멀티히트 헤더
     * "선제 사격! {스킬}({타입}) {N}연타"와 브레이크다운이 생성되는지 검증한다.
     *
     * @param hits      히트 결과 리스트(size 2~8)
     * @param skillType 플레이어 스킬 타입 (NORMAL 또는 HEAVY)
     */
    ...
    void should_produceFirstStrikeMultiHitFormat_when_firstStrikeAndMultiHit(
        ...
        final BattleLogInput input = new BattleLogInput(
                SKILL_LABEL, skillType, MONSTER_NAME, SkillType.NORMAL,
                totalDamage, 0, false, true, false, hits);
        ...
        final String expectedHeader = "선제 사격! " + SKILL_LABEL + "(" + skillType.label() + ") "
                + hits.size() + "연타";
        assertThat(lines.get(0)).isEqualTo(expectedHeader);
    ...
    /**
     * playerHits.size() ≤ 1일 때, 플레이어 로그가 단일 형식 1줄
     * ("{스킬}({타입})로 {몬스터}에게 {N} 피해"(+크리티컬))로 생성되는지 검증한다.
     *
    ...
    void should_produceSingleLineFormat_when_singleHit(
        ...
        final BattleLogInput input = new BattleLogInput(
                SKILL_LABEL, skillType, MONSTER_NAME, SkillType.NORMAL,
                damage, monsterDamage, critical, false, false, hits);
        ...
        final String baseLine = SKILL_LABEL + "(" + skillType.label() + ")로 " + MONSTER_NAME
                + "에게 " + damage + " 피해";
    ...
    void should_produceMissFormat_when_emptyHitsAndZeroDamage(
        ...
        final BattleLogInput input = new BattleLogInput(
                SKILL_LABEL, skillType, MONSTER_NAME, SkillType.NORMAL,
                0, monsterDamage, false, false, false, List.of());
    ...
    /**
     * 브레이크다운 라인의 각 히트 값이 올바르게 "(치명)" 접미사를 가지며
     * 합계가 정확히 일치하는지 검증한다.
     *
     * @param hits 히트 결과 리스트(size 2~8)
     */
    ...
    void should_containCorrectCriticalSuffixes_when_multiHit(
        ...
        final int totalDamage = hits.stream().mapToInt(HitResult::damage).sum();
        final BattleLogInput input = new BattleLogInput(
                SKILL_LABEL, SkillType.NORMAL, MONSTER_NAME, SkillType.NORMAL,
                totalDamage, 5, false, false, false, hits);
    ...
    Arbitrary<List<HitResult>> multiHitResults() {
        final Arbitrary<HitResult> hitArbitrary = Combinators.combine(
