...
class EquipmentKindSlotPropertyTest {
    ...
    /**
     * 양손 무기(양손검·활·스태프)만 {@code requiredSlots.size() == 2}(MAIN_HAND+OFF_HAND)이며,
     * 나머지는 단일 슬롯(size == 1)이다.
     *
     * @param kind 임의의 EquipmentKind 열거 상수
     */
    @Property(tries = 100)
    void should_haveTwoSlotsForTwoHandedWeapons(@ForAll("equipmentKinds") final EquipmentKind kind) {
        final Set<EquipmentKind> twoHandedKinds = Set.of(
                EquipmentKind.TWO_HANDED_SWORD, EquipmentKind.BOW, EquipmentKind.STAFF);
     ...
     * 근접/원거리 무기=MAIN_HAND, 방패=OFF_HAND, 갑옷=BODY, 투구=HEAD, 장갑=HANDS, 부츠=FEET.
     *
     * @param kind 임의의 EquipmentKind 열거 상수
     */
    @Property(tries = 100)
    void should_havePrimarySlotMatchingDefinition(@ForAll("equipmentKinds") final EquipmentKind kind) {
        switch (kind) {
            ...
            case HELMET ->
                    assertThat(kind.primarySlot()).isEqualTo(EquipSlot.HEAD);
            case GLOVES ->
                    assertThat(kind.primarySlot()).isEqualTo(EquipSlot.HANDS);
            case BOOTS ->
                    assertThat(kind.primarySlot()).isEqualTo(EquipSlot.FEET);
        }
    }
