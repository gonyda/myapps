 ...
 * 비인접 이동 거부 프로퍼티 테스트.
 *
 * <p>미연결 방향(좌표에 노드가 없거나, 노드가 있어도 링크되지 않은 경우) 요청 시 {@code Blocked}를 반환하고 현재 노드 id가 변하지 않는지 검증한다.
 *
...
class MovementServiceBlockedMovePropertyTest {
    ...
    /**
     * 오프셋 좌표에 노드가 존재하지 않는 경우(Case A) 이동이 거부되고 현재 노드 id가 변하지 않는지 검증한다.
     *
     * @param scenario 맵 그래프, 출발 노드 ID, 빈 좌표를 가리키는 (dx, dy) 오프셋 튜플
     */
    @Property(tries = 100)
    void should_returnBlocked_when_noNodeAtTargetCoordinates(
            @ForAll("noNeighborAtOffsetScenario")
                    final Tuple.Tuple3<MapGraph, String, Tuple.Tuple2<Integer, Integer>> scenario) {
     ...
     *
     * <p>최소 1개 노드를 가진 그래프를 생성하고, 어떤 노드도 존재하지 않는 좌표를 가리키는 오프셋을 선택한다.
     *
     * @return (MapGraph, 출발 노드 ID, (dx, dy)) 3-튜플 Arbitrary
     */
    @Provide
    Arbitrary<Tuple.Tuple3<MapGraph, String, Tuple.Tuple2<Integer, Integer>>>
            noNeighborAtOffsetScenario() {
     ...
     *
     * <p>최소 3개 노드를 가진 체인 그래프를 생성하고, 출발 노드에서 좌표상 이웃이지만 links에 포함되지 않은 노드를 찾아 오프셋을 계산한다.
     *
     * @return (MapGraph, 출발 노드 ID, (dx, dy)) 3-튜플 Arbitrary
     */
    @Provide
    Arbitrary<Tuple.Tuple3<MapGraph, String, Tuple.Tuple2<Integer, Integer>>>
            unlinkedNeighborScenario() {
        return Arbitraries.integers()
                .between(3, 6)
                .flatMap(
                        nodeCount ->
     ...
     * @return 생성된 노드 목록
     */
    private List<MapNode> generateLinearGraph(
            final int nodeCount, final int baseX, final int baseY) {
    ...
    /**
     * 격자형 그래프에서 첫 번째 노드와 좌표상 인접하지만 링크되지 않은 노드를 포함하는 그래프를 생성한다.
     *
     * <p>노드 0 → 노드 1은 수평 인접+링크, 노드 2는 노드 0과 수직 인접하지만 링크 없음. 나머지 노드는 노드 1과 체인 연결한다.
     *
     ...
     * @return 생성된 노드 목록
     */
    private List<MapNode> generateGridGraphWithGaps(
            final int nodeCount, final int baseX, final int baseY) {
        final List<String> ids = new ArrayList<>();
        for (int i = 0; i < nodeCount; i++) {
            ids.add("grid-" + i);
     ...
     * @return dx 오프셋
     */
    private int computeEmptyOffsetDx(
            final int direction, final int magnitude, final int nodeCount) {
     ...
     * @return dy 오프셋
     */
    private int computeEmptyOffsetDy(
            final int direction, final int magnitude, final int nodeCount) {
        return switch (direction) {
     ...
     */
    private MapNode findUnlinkedCoordNeighbor(final MapNode source, final List<MapNode> nodes) {
        return nodes.stream()
