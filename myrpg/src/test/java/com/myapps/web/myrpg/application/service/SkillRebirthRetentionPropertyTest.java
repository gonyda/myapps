 ...
 * 환생 시 스킬 유지 프로퍼티 테스트.
 *
 * <p>환생({@code CharacterService.rebirth})은 보유 스킬을 변경하지 않는다.
 * 임의의 보유 스킬 집합에 대해, 환생 전후 목록·랭크·카운트가 불변임을 검증한다.
 *
 * <p>현재 구현에서 {@code CharacterService.rebirth}는 {@code CharacterSkill}을 건드리지 않으므로,
 * 이 테스트는 임의의 스킬 목록을 생성하고 스냅샷 비교로 불변성을 확인하는 문서화 테스트이다.
 *
...
class SkillRebirthRetentionPropertyTest {
     ...
     *
     * <p>환생은 {@code CharacterSkill}에 대해 아무 연산을 수행하지 않으므로,
     * 스킬 목록의 각 필드가 환생(= no-op on skills) 전후로 보존됨을 검증한다.
     *
    ...
    void should_retainAllSkillsUnchanged_when_rebirthOccurs(
        ...
        final List<CharacterSkill> ownedSkills = skills.stream()
                .map(snapshot -> new CharacterSkill(
                        CHARACTER_ID, snapshot.skillId(), snapshot.rank(),
                        snapshot.usageCount(), snapshot.killCount()))
                .toList();

        // 스냅샷 저장 (환생 전 상태)
        final List<SkillSnapshot> beforeRebirth = ownedSkills.stream()
                .map(skill -> new SkillSnapshot(
        ...
        for (int i = 0; i < ownedSkills.size(); i++) {
            ...
            assertThat(current.getRank())
                    .as("환생 후 랭크 보존 (index %d)", i)
                    .isEqualTo(snapshot.rank());
    ...
    private Arbitrary<SkillSnapshot> skillSnapshot() {
        final Arbitrary<String> skillIds = Arbitraries.of(
                "windmill", "smash", "magnum_shot", "arrow_revolver",
        ...
        return Combinators.combine(skillIds, ranks, usages, kills)
                .as(SkillSnapshot::new);
    }
