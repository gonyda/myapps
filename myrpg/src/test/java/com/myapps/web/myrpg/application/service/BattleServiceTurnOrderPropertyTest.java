 ...
 *
 * <p>동일 타입 무승부 시 50:50 분포(시드 고정), 일반↔방어는 결정론적으로
 * 공격자(일반) 먼저, 선공 처치 시 후공 피해 0인지 검증한다.
 *
...
class BattleServiceTurnOrderPropertyTest {
    ...
    /**
     * 일반(플레이어) vs 방어(몬스터) 시 결정론적으로 공격자(일반)가 먼저 피해를 주는지 검증한다.
     * 플레이어 NORMAL vs 몬스터 DEFENSE 시 플레이어가 경감된 피해를 먼저, 몬스터가 반격.
     *
     * @param seed 랜덤 시드
     */
    @Property(tries = 100)
    void should_attackerGoFirst_when_normalVsDefense(@ForAll("seeds") final long seed) {
        ...
        assertThat(result.blocked())
                .as("일반 vs 방어에서는 방어(blocked) 플래그가 참이어야 한다")
                .isTrue();
    }

    /**
     * 선공이 후공을 처치하면 후공 피해는 0이어야 한다.
     * 플레이어가 몬스터를 처치할 만큼의 피해를 주는 시나리오.
     * 플레이어 NORMAL vs 몬스터 DEFENSE에서 플레이어가 항상 선공.
     *
     * @param seed 랜덤 시드
     */
    @Property(tries = 100)
    ...
    private BattleService createBattleServiceWithHighDamage(final Random random,
                                                            final SkillType monsterAction) {
    ...
    private BattleService buildService(final Random random,
        ...
        when(rewardService.rollDrop(any(Monster.class)))
                .thenReturn(new DropResult(10L, List.of()));
    ...
    private java.util.Map<SkillRank, Integer> createFullRankMap(final int baseValue) {
        return java.util.Map.ofEntries(
                java.util.Map.entry(SkillRank.F, baseValue), java.util.Map.entry(SkillRank.E, baseValue + 10),
                java.util.Map.entry(SkillRank.D, baseValue + 20), java.util.Map.entry(SkillRank.C, baseValue + 30),
                java.util.Map.entry(SkillRank.B, baseValue + 40), java.util.Map.entry(SkillRank.A, baseValue + 50),
                java.util.Map.entry(SkillRank.R9, baseValue + 60), java.util.Map.entry(SkillRank.R8, baseValue + 70),
                java.util.Map.entry(SkillRank.R7, baseValue + 80), java.util.Map.entry(SkillRank.R6, baseValue + 90),
                java.util.Map.entry(SkillRank.R5, baseValue + 100), java.util.Map.entry(SkillRank.R4, baseValue + 110),
                java.util.Map.entry(SkillRank.R3, baseValue + 120), java.util.Map.entry(SkillRank.R2, baseValue + 130),
                java.util.Map.entry(SkillRank.R1, baseValue + 140), java.util.Map.entry(SkillRank.MASTER, baseValue + 150));
    }
}
