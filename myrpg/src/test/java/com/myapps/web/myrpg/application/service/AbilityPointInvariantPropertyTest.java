 ...
 *
 * <p>소모({@code spendAbilityPoints})가 한 번도 발생하지 않은 진행상황에서, 레벨업과 환생의 임의 시퀀스를 수행한 후 {@code
 * abilityPoints == accumulatedLevel - 1}(AP_Invariant)이 항상 성립하는지 검증한다. 또한 신규 생성 직후 {@code 0 == 1 -
 * 1}을 만족하는지 확인한다.
 *
...
class AbilityPointInvariantPropertyTest {
    ...
    void should_satisfyInvariant_when_createdDefault() {
        ...
        assertThat(progress.getAbilityPoints()).as("신규 생성 시 AP는 0이어야 한다").isEqualTo(0);
        assertThat(progress.getAccumulatedLevel()).as("신규 생성 시 누적레벨은 1이어야 한다").isEqualTo(1);
    ...
    /**
     * 임의의 레벨업·환생 시퀀스(소모 없음)를 수행한 후 AP 정합성 불변식이 매 단계에서 성립하는지 검증한다.
     *
     * <p>시퀀스:
     *
     * <ol>
     *   <li>createDefault() → 불변식 확인
     *   <li>임의 횟수 경험치 획득(레벨업 발생) → 매번 불변식 확인
     *   <li>환생 → 불변식 확인
     *   <li>추가 경험치 획득(레벨업 발생) → 매번 불변식 확인
     * </ol>
     *
    ...
    private void assertInvariant(final CharacterProgress progress, final String context) {
        assertThat(progress.getAbilityPoints())
                .as(
                        "AP_Invariant 위반 (%s): AP=%d, accLv=%d",
