 ...
 *
 * <p>임의의 던전 ID와 임의의 캐릭터 상태(다양한 currentNodeId)에 대해
 * {@code enterDungeon()}이 항상 {@link MovementResult.DungeonLocked}를 반환하고
 * 캐릭터의 현재 노드 ID가 변하지 않는지 검증한다.
 *
...
class MovementServiceDungeonLockedPropertyTest {
    ...
    /**
     * 임의의 던전 ID와 임의의 캐릭터 상태에서 {@code enterDungeon()} 호출 시
     * 항상 {@link MovementResult.DungeonLocked}를 반환하고,
     * 캐릭터의 {@code currentNodeId}가 호출 전과 동일한지 검증한다.
     *
     ...
     *
     * <p>currentNodeId는 실제 맵 노드 ID 형태의 다양한 문자열,
     * dungeonId는 임의의 영문+숫자+하이픈 조합 문자열을 생성한다.
     *
    ...
    Arbitrary<Tuple.Tuple2<String, String>> dungeonEntryScenario() {
        final Arbitrary<String> nodeIds = Arbitraries.of(
                ...
                "node-0", "node-1", "node-99", "field-center",
                "dungeon-entrance-1", "town-square", "forest-path"
        );

        final Arbitrary<String> dungeonIds = Arbitraries.strings()
                .alpha().numeric()
                .ofMinLength(1)
                .ofMaxLength(30)
                .map(s -> "dungeon-" + s);
        ...
        return nodeIds.flatMap(nodeId ->
                dungeonIds.map(dId -> Tuple.of(nodeId, dId))
        );
    }
}
