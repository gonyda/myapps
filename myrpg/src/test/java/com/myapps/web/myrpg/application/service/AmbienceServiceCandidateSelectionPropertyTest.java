 ...
 *
 * <ul>
 *   <li>(a) 후보 존재 시 해당 목록({@code themes[theme][season][tod]})의 원소
 *   <li>(b) 정확한 매치가 없을 때 동일 theme 내 폴백 목록의 원소
 *   <li>(c) theme 후보가 전무할 때 정확히 기본 문구 {@code "{맵이름} 주변을 둘러봅니다."}
 * </ul>
 *
...
class AmbienceServiceCandidateSelectionPropertyTest {
    ...
    private static final List<String> TIME_OF_DAYS =
     ...
     *
     * <p>실제 ambience.json의 모든 theme × season × timeOfDay 조합은 비어 있지 않으므로, 해당 조합에 대응하는 시각을 고정하고 결과가
     * 정확한 매치 목록에 포함되는지 검증한다.
     *
    ...
    void should_returnExactMatchCandidate_when_candidatesExist(
        ...
        final List<String> exactCandidates =
                realAmbienceData.themes().get(theme).get(season).get(timeOfDay);

        assertThat(exactCandidates)
                .as(
                        "정확한 매치 후보 목록이 비어 있지 않아야 함 (theme=%s, season=%s, tod=%s)",
    ...
    /**
     * (b) 정확한 매치가 비어 있지만 동일 theme 내 다른 season/tod에 후보가 존재하면, 결과는 폴백 풀(동일 theme의 모든 멘트)의 원소이다.
     *
     * <p>커스텀 AmbienceData를 주입하여 특정 season+tod 조합을 비워두고, 동일 theme의 다른 조합에만 후보를 배치하여 폴백 동작을 검증한다.
     *
    ...
    void should_returnFallbackCandidate_when_exactMatchIsEmpty(
        ...
        final String otherSeason =
                SEASONS.stream().filter(s -> !s.equals(targetSeason)).findFirst().orElse("summer");
        ...
        final AmbienceData customData =
                createCustomAmbienceDataWithGap(
                        "custom-theme",
                        targetSeason,
                        targetTod,
                        otherSeason,
    ...
    void should_returnDefaultMessage_when_themeHasNoCandidates(
        ...
        assertThat(result).as("theme 후보 전무 시 기본 문구를 반환해야 함").isEqualTo(expectedDefault);
    }
