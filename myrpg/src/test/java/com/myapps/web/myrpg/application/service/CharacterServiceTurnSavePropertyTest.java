 ...
 * 턴 종료 저장 반영 프로퍼티 테스트.
 *
 * <p>성공적 인접 이동(턴 종료) 시 {@code CharacterService.saveTurn()}을 통해 {@code CharacterProgressRepository}에
 * 저장되는 진행상황이 변경된 현재 노드 id를 담는지 검증한다. {@code Mockito.mock()} 리포지토리로 저장 인자를 캡처하여 확인한다.
 *
...
class CharacterServiceTurnSavePropertyTest {
    ...
    /**
     * 성공적 인접 이동 후 턴 종료 저장 시, 리포지토리에 전달되는 {@code CharacterProgress}의 {@code currentNodeId}가 이동 대상 노드
     * id와 일치하는지 검증한다.
     *
    ...
    void should_saveProgressWithTargetNodeId_when_adjacentMoveSucceeds(
        ...
        final ArgumentCaptor<CharacterProgress> captor =
                ArgumentCaptor.forClass(CharacterProgress.class);
