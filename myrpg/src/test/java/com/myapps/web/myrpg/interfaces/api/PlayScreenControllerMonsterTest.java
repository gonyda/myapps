 ...
 *
 * <p>몬스터 조우 엔드포인트({@code POST /monster/encounter}), 이동 후 몬스터 버튼 노출,
...
class PlayScreenControllerMonsterTest {
     ...
     */
    @Test
    void should_returnMonsterResponseWithMonsterSlots_when_encounterKnownMonster() throws Exception {
        ...
        final PlayScreenView monsterView = new PlayScreenView(
                dummyTopBar(),
        ...
        when(playScreenViewHelper.buildPlayScreen(
                any(), any(), any(), anyString(), eq(interactions), any(TalkTarget.class), any(), isNull()))
                .thenReturn(monsterView);

        final MvcResult result = mockMvc.perform(post("/monster/encounter")
                        .param("monsterId", "raccoon"))
    ...
    /**
     * POST /monster/encounter 요청 시 미지 monsterId이면 monster-response가 반환되고
     * 몬스터 슬롯이 모두 null로 비워지는지 검증한다(관용 처리).
     */
    @Test
    void should_returnEmptyMonsterSlots_when_encounterUnknownMonster() throws Exception {
        ...
        final PlayScreenView emptyMonsterView = new PlayScreenView(
                dummyTopBar(),
                new MinimapView("테스트맵", List.of()),
                new FullMapView(List.of(), 5, 5),
                "평화로운 마을",
    ...
    /**
     * POST /monster/encounter 요청 시 NPC 슬롯이 null이고 몬스터 슬롯만 채워지는지 검증한다
     * (NPC·몬스터 슬롯 배타).
     */
    @Test
