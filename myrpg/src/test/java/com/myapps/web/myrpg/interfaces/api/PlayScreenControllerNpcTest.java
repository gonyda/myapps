 ...
 *
 * <p>NPC가 있는 노드에서의 상호작용 버튼 노출, NPC 대화 요청, 노드 이동 시 상호작용 재구성, NPC가 없는 노드 시나리오를 검증한다.
 */
...
class PlayScreenControllerNpcTest {
    ...
    /**
     * GET / 요청 시 NPC가 있는 노드에서 interactions에 NPC 버튼이 노출되고 npc-talk 영역(npcName/npcDialogue)이 비어 있으며
     * npcActions가 null인지 검증한다.
     */
    @Test
    void should_exposeNpcInteractionsAndEmptyTalk_when_nodeHasNpcs() throws Exception {
        ...
        final PlayScreenView viewWithNpcs =
                new PlayScreenView(
                        dummyTopBar(),
    ...
    /**
     * POST /npc/talk?npcId= 요청 시 npcName·대사·npcActions가 올바르게 노출되고 fragments/npc-response 뷰가 반환되는지
     * 검증한다. 이전 내용은 완전히 교체된다(이전 NPC 정보 미포함).
     */
    @Test
    void should_exposeNpcNameDialogueAndActions_when_talkToNpc() throws Exception {
        ...
        final List<ActionButton> npcActions =
                List.of(new ActionButton("상점"), new ActionButton("수리"));
        ...
        when(actionLog.getEntries()).thenReturn(List.of());
        when(playScreenViewHelper.buildPlayScreen(
                        any(), any(), any(), eq(interactions), eq(neris), eq("좋은 물건이 많아요!"), any()))
                .thenReturn(talkView);

        final MvcResult result =
                mockMvc.perform(post("/npc/talk").param("npcId", "neris"))
                        .andExpect(status().isOk())
    ...
    void should_reconstructInteractions_when_moveToNodeWithDifferentNpcs() throws Exception {
        ...
        final List<InteractionItem> newInteractions =
                List.of(new InteractionItem("aranwen", "아란웬 (학교)", true));
        ...
        when(playScreenViewHelper.buildInteractions(newNodeNpcs, List.of()))
                .thenReturn(newInteractions);
        when(monsterEncounterService.rollPreemptiveStrike(anyList())).thenReturn(Optional.empty());
        when(progressionService.rebirthStatus(any(CharacterProgress.class)))
                .thenReturn(new RebirthStatus(true, false, null, null));
        when(playScreenViewHelper.buildInfo(any(CharacterProgress.class), any(RebirthStatus.class)))
                .thenReturn(dummyInfo());
        when(playScreenViewHelper.buildPlayScreen(
                        any(), any(), any(), eq(newInteractions), isNull(), isNull(), any(), any()))
    ...
    void should_returnEmptyInteractions_when_nodeHasNoNpcs() throws Exception {
        ...
        final PlayScreenView emptyView =
                new PlayScreenView(
                        dummyTopBar(),
                        new MinimapView("테스트맵", List.of()),
                        new FullMapView(List.of(), 5, 5),
        ...
        when(playScreenViewHelper.buildInfo(any(CharacterProgress.class), any(RebirthStatus.class)))
                .thenReturn(dummyInfo());
        when(playScreenViewHelper.buildPlayScreen(
                        any(), any(), any(), eq(List.of()), isNull(), isNull(), any(), any()))
    ...
    private Npc createNpc(
            final String id, final String name, final NpcType type, final String nodeId) {
