 ...
 *
 * <p>50% 성공(active=false, outcome=FLED), 실패(몬스터 1회 피해, 전투 유지), 실패로 HP 0이면 사망 처리로 이어지는지 검증한다.
 *
...
class BattleServiceFleePropertyTest {
    ...
    /**
     * 도망 성공 시 active=false, outcome=FLED를 검증한다. random.nextInt(100) < 50 이면 도망 성공.
     *
    ...
    void should_fleeSuccessfully_when_rollBelow50(@ForAll("successSeeds") final long successSeed) {
        ...
        assertThat(result.outcome()).as("도망 성공 시 outcome은 FLED여야 한다").isEqualTo(Outcome.FLED);
        assertThat(result.battleEnded()).as("도망 성공 시 전투가 종료되어야 한다").isTrue();
        assertThat(state.isActive()).as("도망 성공 시 BattleState active는 false여야 한다").isFalse();
    }

    /**
     * 도망 실패 시 몬스터 1회 피해 적용 후 전투 유지를 검증한다. random.nextInt(100) >= 50 이면 도망 실패.
     *
    ...
    void should_takeDamageAndContinue_when_fleeFail(@ForAll("failSeeds") final long failSeed) {
        ...
        assertThat(result.monsterDamage()).as("도망 실패 시 몬스터 1회 피해가 있어야 한다").isGreaterThan(0);
        assertThat(result.outcome()).as("도망 실패 시 HP가 남아있으면 전투 계속(NONE)").isEqualTo(Outcome.NONE);
        assertThat(state.isActive()).as("도망 실패 시 전투가 계속되어야 한다").isTrue();
    }
     ...
     */
    @Property(tries = 100)
    void should_triggerDeath_when_fleeFailAndHpReachesZero(
    ...
    private BattleService buildService(final Random random, final boolean deathScenario) {
        ...
        when(resolver.finalDamage(anyInt(), anyDouble(), any(Boolean.class)))
                .thenReturn(MONSTER_DAMAGE);
