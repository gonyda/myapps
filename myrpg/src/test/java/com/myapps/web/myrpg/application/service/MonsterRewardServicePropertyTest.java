 ...
 *
 * <p>임의의 {@link GoldDrop}과 roll 값에 대해 {@code goldFor}가 항상 {@code [min, max]} 범위의 값을 반환하고, 아이템 확률
 * 경계(100%/1%)와 수량 범위를 검증한다.
 *
...
class MonsterRewardServicePropertyTest {
    ...
    /**
     * 임의의 유효한 {@code GoldDrop(min, max)}과 roll 값에 대해, {@code goldFor}는 항상 {@code [min, max]} 범위의 값을
     * 반환한다.
     *
    ...
    /**
     * {@code min == max}인 {@code GoldDrop}에 대해, {@code goldFor}는 roll 값에 관계없이 항상 그 고정값을 반환한다.
     *
    ...
    /**
     * {@code chancePercent=100}인 아이템은 {@code rollDrop}에서 항상 드랍된다. 내부 Random이 0~99 모든 값을 반환해도 100%
     * 아이템은 반드시 포함된다.
     *
    ...
    void should_alwaysDrop_when_chancePercentIs100(
            @ForAll("seeds") final long seed, @ForAll("validGoldDrop") final GoldDrop goldDrop) {
        ...
        assertThat(result.items()).extracting(DroppedItem::itemId).contains(itemId);
    }

    /**
     * {@code chancePercent=1}이고 Random이 99를 반환하면 아이템이 드랍되지 않는다. (chancePercent 범위가 1~100이므로 0은
     * 테스트하지 않음)
     *
     * @param goldDrop 임의 생성된 유효 골드 드랍 범위
     */
    ...
    void should_notDrop_when_chancePercent1AndRollIs99(
        ...
        assertThat(result.items()).extracting(DroppedItem::itemId).doesNotContain(itemId);
    }
    ...
    Arbitrary<GoldDrop> validGoldDrop() {
        return Arbitraries.integers()
                .between(0, GOLD_MAX_UPPER)
                .flatMap(
                        min ->
                                Arbitraries.integers()
                                        .between(min, min + GOLD_MAX_UPPER)
    ...
    Arbitrary<int[]> validQuantityRange() {
        return Arbitraries.integers()
                .between(1, QUANTITY_MAX_UPPER)
                .flatMap(
                        min ->
                                Arbitraries.integers()
                                        .between(min, QUANTITY_MAX_UPPER)
