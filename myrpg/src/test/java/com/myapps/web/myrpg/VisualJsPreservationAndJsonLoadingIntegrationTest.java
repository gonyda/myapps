...
class VisualJsPreservationAndJsonLoadingIntegrationTest {
    ...
    private static final List<String> CSS_DESIGN_TOKENS =
            List.of(
                    "--gap",
                    "--pad",
                    "--radius",
    ...
    private static final List<String> JS_ZOOM_PAN_FUNCTIONS =
            List.of("applyMapTransform", "zoomAt", "mapZoom");

    private static final List<String> JS_POPUP_FUNCTIONS =
            List.of(
                    "openPanel",
                    "closePanel",
                    "openMap",
    ...
    private static final List<String> JS_MONSTER_FUNCTIONS =
            List.of("swapCenter", "onInteractionClick", "encounterMonster", "startBattle");

    private static final List<String> JS_BATTLE_FUNCTIONS =
            List.of(
                    "battleActive",
                    "startBattle",
                    "startClash",
                    ...
                    "flee",
                    "fetchBattleView",
                    "handleTurnResultSignal",
                    "refreshBattleSkills");

    private static final List<String> CENTER_MONSTER_MARKERS =
            List.of(
                    "monster-name",
                    "monster-meta",
                    "monster-actions",
    ...
    void should_containDesignTokens_when_cssMyrpgLoaded() throws IOException {
        ...
        for (final String token : CSS_DESIGN_TOKENS) {
            assertThat(css).as("CSS 디자인 토큰 '%s' 존재 확인", token).contains(token);
        }
    }
    ...
    void should_containZoomPanFunctions_when_jsMyrpgLoaded() throws IOException {
        ...
        for (final String function : JS_ZOOM_PAN_FUNCTIONS) {
            assertThat(js).as("줌/팬 함수 '%s' 존재 확인", function).contains(function);
        }
    }
    ...
    void should_containPopupFunctions_when_jsMyrpgLoaded() throws IOException {
        ...
        for (final String function : JS_POPUP_FUNCTIONS) {
            assertThat(js).as("팝업 함수 '%s' 존재 확인", function).contains(function);
        }
    }
    ...
    void should_containNpcActionFunctions_when_jsMyrpgLoaded() throws IOException {
        ...
        for (final String function : JS_NPC_ACTION_FUNCTIONS) {
            assertThat(js).as("NPC 액션 JS 함수 '%s' 존재 확인", function).contains(function);
        }
    }
    ...
    void should_containMonsterFunctions_when_jsMyrpgLoaded() throws IOException {
        ...
        for (final String function : JS_MONSTER_FUNCTIONS) {
            assertThat(js).as("몬스터 JS 함수 '%s' 존재 확인", function).contains(function);
        }
    }
    ...
    void should_containBattleFunctions_when_jsMyrpgLoaded() throws IOException {
        ...
        for (final String function : JS_BATTLE_FUNCTIONS) {
            assertThat(js).as("전투 JS 함수/변수 '%s' 존재 확인", function).contains(function);
        }
    }
    ...
    void should_containMonsterMarkup_when_centerHtmlLoaded() throws IOException {
        ...
        for (final String marker : CENTER_MONSTER_MARKERS) {
            assertThat(centerHtml).as("center.html 몬스터 마커 '%s' 존재 확인", marker).contains(marker);
        }
    }
    ...
    void should_existAllFragments_when_templateDirectoryChecked() {
        for (final String fragmentPath : EXPECTED_FRAGMENTS) {
            ...
            assertThat(resource.exists()).as("Fragment '%s' 존재 확인", fragmentPath).isTrue();
        }
    }
        ...
        try (InputStream inputStream = resource.getInputStream()) {
            ...
            for (final Map.Entry<String, List<String>> entry : linkMap.entrySet()) {
                ...
                for (final String linkedId : entry.getValue()) {
                    assertThat(linkMap).as("링크 대상 '%s'가 맵에 존재해야 함", linkedId).containsKey(linkedId);
                    assertThat(linkMap.get(linkedId))
                            .as(
                                    "양방향 링크: '%s' → '%s' 존재 시 '%s' → '%s'도 존재해야 함",
    ...
    /** skill.json을 Jackson 3로 로드하여 11개 스킬이 포함되고 각 스킬에 id와 type 필드가 존재하는지 검증한다. */
    @Test
    void should_loadAllSkills_when_skillJsonParsed() throws IOException {
        ...
        try (InputStream inputStream = resource.getInputStream()) {
            ...
            for (final JsonNode skillNode : root) {
                assertThat(skillNode.has("id")).as("스킬 노드에 'id' 필드가 존재해야 함").isTrue();
                assertThat(skillNode.has("type")).as("스킬 노드에 'type' 필드가 존재해야 함").isTrue();
            }
        }
    }
