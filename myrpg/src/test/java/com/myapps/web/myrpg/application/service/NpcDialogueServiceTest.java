...
class NpcDialogueServiceTest {
    ...
    void should_selectFromDefaultAndTimeLines_when_bothExist() {
        ...
        final Clock fixedClock = Clock.fixed(Instant.parse("2025-01-01T10:00:00Z"), ZoneId.of("UTC"));
        final NpcDialogueService service = new NpcDialogueService(fixedClock, seededRandom);

        final NpcLines lines = new NpcLines(
                List.of("기본1", "기본2"),
                Map.of("morning", List.of("오전1", "오전2"))
        );
    ...
    void should_useClockHour_when_selectLineWithoutHour() {
        final Clock fixedClock = Clock.fixed(
                Instant.parse("2025-06-15T22:30:00Z"), ZoneId.of("UTC"));
