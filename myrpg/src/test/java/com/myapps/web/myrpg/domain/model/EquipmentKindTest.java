...
class EquipmentKindTest {
    ...
    /** ONE_HANDED_SWORD의 라벨은 "한손검", primarySlot은 MAIN_HAND, requiredSlots는 {MAIN_HAND}임을 검증한다. */
    @Test
    ...
    /**
     * TWO_HANDED_SWORD의 라벨은 "양손검", primarySlot은 MAIN_HAND, requiredSlots는 {MAIN_HAND, OFF_HAND}임을
     * 검증한다.
     */
    @Test
    ...
    /** SHIELD의 라벨은 "방패", primarySlot은 OFF_HAND, requiredSlots는 {OFF_HAND}임을 검증한다. */
    @Test
    ...
    /** ARMOR_BODY의 라벨은 "갑옷", primarySlot은 BODY, requiredSlots는 {BODY}임을 검증한다. */
    @Test
    ...
    void should_returnOneHandedSword_when_fromStringWithValidCode() {
        assertThat(EquipmentKind.fromString("ONE_HANDED_SWORD"))
                .isPresent()
    ...
    void should_returnTwoHandedSword_when_fromStringWithValidCode() {
        assertThat(EquipmentKind.fromString("TWO_HANDED_SWORD"))
                .isPresent()
    ...
    void should_returnShield_when_fromStringWithValidCode() {
        assertThat(EquipmentKind.fromString("SHIELD")).isPresent().contains(EquipmentKind.SHIELD);
        assertThat(EquipmentKind.fromString("shield")).isPresent().contains(EquipmentKind.SHIELD);
    }
    ...
    void should_returnArmorBody_when_fromStringWithValidCode() {
        assertThat(EquipmentKind.fromString("ARMOR_BODY"))
                .isPresent()
