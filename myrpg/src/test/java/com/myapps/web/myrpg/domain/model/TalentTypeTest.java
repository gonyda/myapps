 ...
 *
 * <p>3개 상수({@code MELEE}, {@code ARCHERY}, {@code MAGIC})의 {@code label()}, {@code
 * primary()}/{@code secondary()}/{@code damageBonusPercent()}/{@code effectSummary()} 구체값과 {@link
 * TalentType#fromNameOrFallback(String, TalentType)} 폴백 동작을 예시로 확인한다.
 *
...
class TalentTypeTest {
    ...
    void should_returnCriticalSecondary_when_archery() {
        assertThat(TalentType.ARCHERY.secondary())
                .isEqualTo(new TalentBonus(BonusTarget.CRITICAL, 1));
    }
    ...
    void should_returnArcheryEffectSummary_when_archery() {
        assertThat(TalentType.ARCHERY.effectSummary())
                .isEqualTo("원거리 데미지 +10%, DEX +2/Lv, 치명 +0.1%/Lv");
    }
    ...
    void should_returnArchery_when_validName() {
        assertThat(TalentType.fromNameOrFallback("ARCHERY", TalentType.MELEE))
                .isEqualTo(TalentType.ARCHERY);
    }

    /** 이름이 null이면 폴백 재능을 반환함을 검증한다. */
    @Test
    void should_returnFallback_when_nullName() {
        assertThat(TalentType.fromNameOrFallback(null, TalentType.MELEE))
                .isEqualTo(TalentType.MELEE);
    }
    ...
    void should_returnFallback_when_unknownName() {
        assertThat(TalentType.fromNameOrFallback("XXX", TalentType.MELEE))
                .isEqualTo(TalentType.MELEE);
    }
}
