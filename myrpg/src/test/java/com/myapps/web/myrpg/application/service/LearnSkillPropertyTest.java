 ...
 * <ul>
 *   <li>카탈로그에 존재 + 미보유 → F 랭크·카운트 0으로 추가 (save 호출)</li>
 *   <li>이미 보유 → 변경 없음 (save 미호출)</li>
 *   <li>카탈로그에 없음 → 추가하지 않음 (save 미호출)</li>
 * </ul>
 *
...
class LearnSkillPropertyTest {
    ...
    void should_addSkillAtRankF_when_catalogHasAndNotOwned(
        ...
        verify(mockRepo).save(org.mockito.ArgumentMatchers.argThat(saved ->
     ...
     */
    @Property(tries = 100)
    void should_notChange_when_alreadyOwned(
            @ForAll("catalogSkillId") final String skillId) {
        ...
        final CharacterSkill existingSkill = new CharacterSkill(
                CHARACTER_ID, skillId, SkillRank.D, 50, 20);
        when(mockRepo.findByCharacterIdAndSkillId(CHARACTER_ID, skillId))
     ...
     */
    @Property(tries = 100)
    void should_notAdd_when_catalogDoesNotHave(
            @ForAll("unknownSkillId") final String skillId) {
    ...
    Arbitrary<String> unknownSkillId() {
        return Arbitraries.of(
                "unknown_skill", "nonexistent", "fake_bolt",
                "invalid_skill", "no_such_skill", "random_xxx");
    }
    ...
    private Skill createDummySkill(final String skillId) {
        ...
        return new DamageSkill(skillId, "테스트 스킬", SkillType.NORMAL,
                SkillTalent.MELEE, 10, multiplierByRank, "테스트 효과");
    }
}
