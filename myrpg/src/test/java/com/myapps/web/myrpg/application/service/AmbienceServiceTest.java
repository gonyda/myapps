 ...
 *
 * <p>실제 {@code classpath:data/ambience.json}을 로드하여
 * 계절/시간대 매핑, 테마 결정, 폴백 규칙을 검증한다.
 */
class AmbienceServiceTest {
    ...
    void should_useNodeTheme_when_themeIsPresent() {
        final MapNode node = new MapNode(
                "test-node", "테스트 마을", "field", NodeType.FIELD,
                0, 0, null, "town", List.of());

        final String result = ambienceService.ambience(node);
        final List<String> townSpringMorning = ambienceService.ambienceData()
                .themes().get("town").get("spring").get("morning");
    ...
    void should_useNodeType_when_themeIsNull() {
        final MapNode node = new MapNode(
                "test-node", "테스트 필드", "field", NodeType.FIELD,
                0, 0, null, null, List.of());

        final String result = ambienceService.ambience(node);
        final List<String> fieldSpringMorning = ambienceService.ambienceData()
                .themes().get("field").get("spring").get("morning");
    ...
    void should_returnDefaultMessage_when_themeIsUnknown() {
        final MapNode node = new MapNode(
                "test-node", "미지의 땅", "unknown-type", null,
                0, 0, null, null, List.of());
