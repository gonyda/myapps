 ...
 *
 * <p>가위바위보 9칸 매트릭스의 각 결과와 특수 턴(활 1턴 선제 사격, 마법 캐스팅 실패)에 대해
 * 플레이어·몬스터 로그 문구가 정확히 생성되는지 확인한다. 방어 승리 상황은
 * 플레이어·몬스터 어느 쪽이 방어하든 "방어하며 반격!"으로 대칭 표현되는지 검증한다.
 * 멀티히트(hitCount ≥ 2) 시 헤더+브레이크다운 형식이 올바르게 출력되는지 검증한다.
 */
class BattleLogFormatterTest {
    ...
    void should_logHitAndMonsterAttack_when_normalAttackLands() {
        final BattleLogInput input = new BattleLogInput(
                ATTACK_SKILL, SkillType.NORMAL, MONSTER_NAME, SkillType.NORMAL,
                10, 13, false, false, false, List.of(new HitResult(10, false)));

        final List<String> lines = formatter.combatLines(input);

        assertThat(lines).containsExactly(
                "윈드밀(일반)로 너구리에게 10 피해",
                "너구리의 일반공격, 13 피해를 입음");
    }
    ...
    void should_appendCriticalMark_when_playerCritical() {
        final BattleLogInput input = new BattleLogInput(
                ATTACK_SKILL, SkillType.NORMAL, MONSTER_NAME, SkillType.HEAVY,
                15, 0, true, false, false, List.of(new HitResult(15, true)));

        final List<String> lines = formatter.combatLines(input);

        assertThat(lines).containsExactly(
                "윈드밀(일반)로 너구리에게 15 피해 (크리티컬!)",
                "너구리의 강공격이 빗나갔다!");
    }
    ...
    void should_logPlayerMiss_when_playerAttackDealsZero() {
        final BattleLogInput input = new BattleLogInput(
                HEAVY_SKILL, SkillType.HEAVY, MONSTER_NAME, SkillType.NORMAL,
                0, 13, false, false, false, List.of());
    ...
    void should_logMonsterMiss_when_monsterAttackDealsZero() {
        final BattleLogInput input = new BattleLogInput(
                ATTACK_SKILL, SkillType.NORMAL, MONSTER_NAME, SkillType.HEAVY,
                10, 0, false, false, false, List.of(new HitResult(10, false)));
    ...
    void should_logMonsterCounter_when_monsterDefenseWins() {
        final BattleLogInput input = new BattleLogInput(
                ATTACK_SKILL, SkillType.NORMAL, MONSTER_NAME, SkillType.DEFENSE,
                3, 7, false, false, false, List.of(new HitResult(3, false)));
    ...
    void should_logPlayerCounter_when_playerDefenseWins() {
        final BattleLogInput input = new BattleLogInput(
                DEFENSE_SKILL, SkillType.DEFENSE, MONSTER_NAME, SkillType.NORMAL,
                9, 6, false, false, false, List.of());

        final List<String> lines = formatter.combatLines(input);

        assertThat(lines).containsExactly(
                "디펜스(방어)로 방어하며 반격! (9 피해)",
                "너구리의 일반공격, 6 피해를 입음");
    }
    ...
    void should_logMonsterDefenseBroken_when_heavyPenetratesMonsterDefense() {
        final BattleLogInput input = new BattleLogInput(
                HEAVY_SKILL, SkillType.HEAVY, MONSTER_NAME, SkillType.DEFENSE,
                18, 0, false, false, false, List.of(new HitResult(18, false)));
    ...
    void should_logPlayerDefenseBroken_when_monsterHeavyPenetrates() {
        final BattleLogInput input = new BattleLogInput(
                DEFENSE_SKILL, SkillType.DEFENSE, MONSTER_NAME, SkillType.HEAVY,
                0, 20, false, false, false, List.of());
    ...
    void should_logStalemate_when_bothDefend() {
        final BattleLogInput input = new BattleLogInput(
                DEFENSE_SKILL, SkillType.DEFENSE, MONSTER_NAME, SkillType.DEFENSE,
                0, 0, false, false, false, List.of());
    ...
    void should_logSingleFirstStrikeLine_when_firstStrike() {
        final BattleLogInput input = new BattleLogInput(
                ATTACK_SKILL, SkillType.NORMAL, MONSTER_NAME, SkillType.NORMAL,
                12, 0, false, true, false, List.of(new HitResult(12, false)));
    ...
    void should_logOnlyMonsterAttack_when_castFailureAndMonsterAttacks() {
        final BattleLogInput input = new BattleLogInput(
                MAGIC_SKILL, SkillType.NORMAL, MONSTER_NAME, SkillType.NORMAL,
                0, 13, false, false, true, List.of());
    ...
    void should_logOnlyMonsterDefenseStance_when_castFailureAndMonsterDefends() {
        final BattleLogInput input = new BattleLogInput(
                MAGIC_SKILL, SkillType.NORMAL, MONSTER_NAME, SkillType.DEFENSE,
                0, 0, false, false, true, List.of());
    ...
    void should_logMultiHitHeaderAndBreakdown_when_threeHits() {
        final List<HitResult> hits = List.of(
                new HitResult(22, false), new HitResult(33, true), new HitResult(19, false));
        final BattleLogInput input = new BattleLogInput(
                ATTACK_SKILL, SkillType.NORMAL, MONSTER_NAME, SkillType.NORMAL,
                74, 10, false, false, false, hits);
    ...
    void should_appendCriticalToAllHits_when_allCritical() {
        final List<HitResult> hits = List.of(
                new HitResult(30, true), new HitResult(28, true), new HitResult(32, true));
        final BattleLogInput input = new BattleLogInput(
                HEAVY_SKILL, SkillType.HEAVY, MONSTER_NAME, SkillType.HEAVY,
                90, 0, false, false, false, hits);

        final List<String> lines = formatter.combatLines(input);

        assertThat(lines).containsExactly(
                "스매시(강) 3연타",
                "30(치명)  28(치명)  32(치명) = 90 피해",
                "너구리의 강공격이 빗나갔다!");
    }
}
