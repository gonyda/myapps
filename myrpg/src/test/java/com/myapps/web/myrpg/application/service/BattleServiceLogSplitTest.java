 ...
 *
 * <p>승리 턴: 결산 라인만 {@code actionLog.add} 호출, 액션 라인 미추가. 시작 메서드: 하단 시작 로그 미추가.
 */
class BattleServiceLogSplitTest {
    ...
    void setUp() {
        ...
        when(itemCatalogService.byId("potion")).thenReturn(Optional.empty());
    ...
    void should_onlyAddSettlementToActionLog_when_victoryTurn() {
        ...
        assertThat(result.combatLines()).as("combatLines에 전투 액션 라인이 있어야 한다").isNotEmpty();

        final List<ActionLogEntry> entries = actionLog.getEntries();
        assertThat(entries).as("actionLog에 결산 라인이 추가되어야 한다").isNotEmpty();

        final boolean allSettlement =
                entries.stream()
                        .allMatch(
                                e ->
    ...
    void should_notAddToActionLog_when_normalNonKillTurn() {
        ...
        assertThat(actionLog.size()).as("비처치 일반 턴에서 actionLog에 아무것도 추가되지 않아야 한다").isZero();
    }
    ...
    void should_notAddStartLogToActionLog_when_battleStarts() {
        ...
        assertThat(actionLog.size()).as("start()에서 actionLog에 아무것도 추가되지 않아야 한다").isZero();
    }
    ...
    void should_useLogTypeCombat_when_settlementLinesAdded() {
        ...
        final boolean allCombatType =
                actionLog.getEntries().stream().allMatch(e -> "combat".equals(e.type()));
        assertThat(allCombatType).as("결산 라인의 타입은 'combat'이어야 한다").isTrue();
    }
