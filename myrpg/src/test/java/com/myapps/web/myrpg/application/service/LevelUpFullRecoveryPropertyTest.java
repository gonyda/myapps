 ...
 *
 * <p>현재 바이탈을 낮은 값으로 설정한 뒤 경험치를 부여하여 레벨업을 유발하고, 레벨업 후 HP/MP/Stamina 현재치가 {@code vitalMaxFor(최종 레벨,
 * talent)}의 각 대응 필드와 같음을 확인한다.
 *
...
class LevelUpFullRecoveryPropertyTest {
     ...
     *
     * <p>바이탈을 낮은 값(1~50)으로 설정하여 회복이 확실히 일어났음을 증명한다. 획득 경험치는 최소 1회 레벨업을 보장하도록 생성한다.
     *
    ...
    void should_fullRecoverVitals_when_levelUpOccurs(
        ...
        final VitalMax expectedVitalMax =
                statProgression.vitalMaxFor(result.newLevel(), TalentType.MELEE);
