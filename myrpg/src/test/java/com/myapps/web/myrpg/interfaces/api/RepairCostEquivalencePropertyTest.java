 ...
 *
 * <p>수리 가능한 장비(내구도가 닳은 장비)에 대해, 수리 목록에 노출되는
 * {@code repairCost}는 해당 장비의 {@code ShopService.sellValueOf}와
 * 정확히 일치함을 검증한다.
 *
...
class RepairCostEquivalencePropertyTest {
    ...
    /**
     * 임의의 판매가 계산이 가능한 장비(가중치 합산 또는 buyPrice 경로)에 대해,
     * 수리 목록의 {@code repairCost}가 실제 {@code sellValueOf(ownedItem)}와 일치하는지 검증한다.
     *
     * @param equip 임의 장비 아이템
     */
    @Property(tries = 100)
    void should_makeRepairCostEqualToSellValue(@ForAll("repairableEquip") final EquipmentItem equip) {
        final OwnedItem owned = new OwnedItem(
                equip.id(), 1, StorageKind.INVENTORY, false, MAX_DURABILITY - 5);
    ...
    void should_excludeFullDurabilityEquip(@ForAll("repairableEquip") final EquipmentItem equip) {
        final OwnedItem ownedFull = new OwnedItem(
                equip.id(), 1, StorageKind.INVENTORY, false, equip.maxDurability());
    ...
    Arbitrary<EquipmentItem> repairableEquip() {
        final Arbitrary<EquipmentItem> bonusEquip = Arbitraries.integers().between(1, 100)
                .map(amount -> new EquipmentItem(
                        "bonus-sword-" + amount, "보너스검" + amount, ItemType.WEAPON,
                        EquipmentKind.ONE_HANDED_SWORD,
                        List.of(new EquipBonus(BonusTarget.STR, amount)),
                        null, MAX_DURABILITY));
        final Arbitrary<EquipmentItem> buyPriceEquip = Arbitraries.integers().between(1, 100_000)
                .map(price -> new EquipmentItem(
                        "shop-sword-" + price, "상점검" + price, ItemType.WEAPON,
                        EquipmentKind.ONE_HANDED_SWORD, List.of(),
                        price, MAX_DURABILITY));
    ...
    private Fixture newFixture(final Item catalogItem, final OwnedItem owned) {
        ...
        when(catalogService.byId(anyString()))
                .thenAnswer(invocation -> Optional.of(catalogItem));
