 ...
 * 계절/시간대 매핑 프로퍼티 테스트.
 *
 * <p>월(1~12)에 대해 {@code resolveSeason}이 ambience.json의 season 정의에 부합하는 계절 키를 산출하고, 시(0~23)에 대해
 * {@code resolveTimeOfDay}가 자정을 넘는 구간(late-night)을 포함하여 timeOfDay 정의에 부합하는 시간대 키를 산출하는지 검증한다.
 *
...
class AmbienceServiceSeasonTimeMappingPropertyTest {
    ...
    /**
     * 월(1~12)에 대해 resolveSeason이 반환하는 계절 키가 ambience.json의 season 매핑에서 해당 월을 포함하는 키와 일치하는지 검증한다.
     *
     * @param month 임의의 월 값 (1~12)
     */
    @Property(tries = 100)
    void should_returnMatchingSeason_when_monthIsValid(@ForAll("validMonths") final int month) {
        // When
    ...
    /**
     * 시(0~23)에 대해 resolveTimeOfDay가 반환하는 시간대 키가 ambience.json의 timeOfDay 매핑에서 해당 시각이 [from, to) 범위에
     * 속하는 키와 일치하는지 검증한다. late-night(0~4시) 구간을 포함한다.
     *
     * @param hour 임의의 시각 값 (0~23)
     */
    @Property(tries = 100)
    void should_returnMatchingTimeOfDay_when_hourIsValid(@ForAll("validHours") final int hour) {
        // When
        ...
        assertThat(hour).isGreaterThanOrEqualTo(bucket.from()).isLessThan(bucket.to());
    }

    /**
     * late-night 구간(시각 0~4)에 대해 resolveTimeOfDay가 정확히 "late-night" 키를 반환하는지 검증한다.
     *
