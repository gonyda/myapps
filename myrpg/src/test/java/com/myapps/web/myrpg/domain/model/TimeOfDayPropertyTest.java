 ...
 *
 * <p>{@code hour ∈ [0,23]} 전 구간에서 {@link TimeOfDay#fromHour(int)}가
 * {@code from <= hour < to}를 만족하는 상수를 정확히 하나 반환하며,
 * 6개 구간이 {@code [0,24)}를 빈틈·중복 없이 분할함을 검증한다.
 *
...
class TimeOfDayPropertyTest {
    ...
    /**
     * 임의의 시(hour)에 대해 {@code fromHour(hour)}가 반환하는 {@link TimeOfDay}의
     * 구간 {@code [from, to)}이 해당 hour를 포함하는지 검증한다.
     *
    ...
    /**
     * 임의의 시(hour)에 대해 정확히 하나의 {@link TimeOfDay} 상수만
     * 해당 hour를 구간에 포함하는지 검증한다.
     *
     * @param hour 0 이상 24 미만의 임의 시각
     */
    @Property(tries = 100)
    void should_matchExactlyOneConstant_when_anyValidHour(@ForAll("hours") final int hour) {
        final long matchCount = Arrays.stream(TimeOfDay.values())
    ...
    /**
     * 6개의 {@link TimeOfDay} 구간이 {@code [0, 24)}를 빈틈·중복 없이 분할하는지 검증한다.
     * 구간을 {@code from} 기준 정렬 후, 첫 구간은 0에서 시작하고 마지막 구간은 24에서 끝나며,
     * 인접 구간의 {@code to}와 다음 구간의 {@code from}이 정확히 일치함을 확인한다.
     */
    @Property(tries = 100)
    void should_partitionFullDay_when_allConstantsExamined() {
        final List<TimeOfDay> sorted = Arrays.stream(TimeOfDay.values())
