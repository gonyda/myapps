...
class SkillRankUpEffectPropertyTest {
    ...
    void should_applyTransactionEffects_when_rankUpSucceeds(
        ...
        // Then: 트랜잭션 효과 검증
        assertThat(result).as("Rankable 상태에서 랭크업은 성공해야 한다").isTrue();

        assertThat(skill.getRank()).as("랭크는 이전 랭크의 next()여야 한다").isEqualTo(expectedNextRank);
    ...
    Arbitrary<RankableState> rankableStates() {
        final SkillRank[] nonMasterRanks =
                Arrays.stream(SkillRank.values())
        ...
        return Arbitraries.of(nonMasterRanks)
                .flatMap(
                        rank -> {
                            ...
                            final Arbitrary<Integer> usageArbitrary =
