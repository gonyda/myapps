 ...
 *
 * <p>{@code L ∈ [1,100]} 전 구간에서 {@link ExperiencePolicy#requiredForNext(int)}가
 * {@code 100 × L²} 공식을 정확히 만족하며, 레벨이 증가할수록 필요 경험치가 단조 증가함을 검증한다.
 *
...
class ExperiencePolicyCurvePropertyTest {
    ...
    /**
     * 임의의 레벨 L(1~100)에 대해 {@code requiredForNext(L) == 100 × L × L}임을 검증한다.
     *
     * @param level 1 이상 100 이하의 임의 레벨
     */
    @Property(tries = 100)
    void should_returnHundredTimesLevelSquared_when_anyValidLevel(@ForAll("levels") final int level) {
        final long expected = 100L * level * level;
    ...
    /**
     * 임의의 레벨 L(1~99)에 대해 {@code requiredForNext(L) < requiredForNext(L+1)}임을 검증한다.
     * 경험치 곡선이 단조 증가하는지 확인한다.
     *
     * @param level 1 이상 99 이하의 임의 레벨
     */
    @Property(tries = 100)
    void should_beMonotonicallyIncreasing_when_levelIncreases(@ForAll("levelsForMonotonic") final int level) {
    ...
    /**
     * 단조 증가 검증에 사용할 레벨(1~99)을 생성하는 Arbitrary 제공자.
     * {@code L+1}이 100을 초과하지 않도록 상한을 99로 제한한다.
     *
