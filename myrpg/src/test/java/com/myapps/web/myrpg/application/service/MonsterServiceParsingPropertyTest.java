 ...
 *
 * <p>유효한 몬스터 데이터셋을 JSON으로 직렬화한 뒤 {@code MonsterService.loadFromStream}으로 파싱하여 모든 Monster의 필드가 순서까지
 * 보존되고, {@code itemDrops} 미기재 시 빈 목록이 됨을 검증한다.
 *
...
class MonsterServiceParsingPropertyTest {
    ...
    /**
     * 유효한 몬스터 데이터셋을 JSON으로 직렬화한 뒤 {@code MonsterService.loadFromStream}으로 파싱하면, 항목 수가 보존되고 모든
     * 필드(id, name, type, level, maxHp, attackPower, defense, critical, experience, goldDrop,
     * itemDrops, lines)가 정확히 보존됨을 검증한다.
     *
    ...
    void should_preserveAllFields_when_validDatasetParsed(
        ...
        for (int i = 0; i < dataset.size(); i++) {
            ...
            final MonsterType expectedType =
                    MonsterType.fromType(original.typeString()).orElseThrow();
            ...
            for (int j = 0; j < original.itemDrops().size(); j++) {
                ...
                assertThat(monster.itemDrops().get(j).chancePercent())
                        .isEqualTo(expectedDrop.chancePercent());
                assertThat(monster.itemDrops().get(j).minQuantity())
                        .isEqualTo(expectedDrop.minQuantity());
                assertThat(monster.itemDrops().get(j).maxQuantity())
                        .isEqualTo(expectedDrop.maxQuantity());
            }
    ...
    private Arbitrary<MonsterInputData> monsterInputDataArbitrary() {
        ...
        return Combinators.combine(ids, names, types, statGroup, itemDrops, lines)
                .as(
                        (id, name, type, stats, drops, linesList) ->
                                new MonsterInputData(
                                        id,
                                        name,
                                        type,
    ...
    private Arbitrary<ItemDropInputData> itemDropArbitrary() {
        final Arbitrary<String> itemIds =
                Arbitraries.of("hp_potion_50", "iron_sword", "leather_armor", "mp_potion_30");
        final Arbitrary<Integer> chances = Arbitraries.integers().between(1, 100);
        final Arbitrary<int[]> quantities =
                Arbitraries.integers()
                        .between(1, 5)
                        .flatMap(
                                min ->
        ...
        return Combinators.combine(itemIds, chances, quantities)
                .as((itemId, chance, qty) -> new ItemDropInputData(itemId, chance, qty[0], qty[1]));
    }
    ...
    private ObjectNode buildMonsterNode(final ArrayNode rootArray, final MonsterInputData data) {
    ...
    private void addItemDropsToNode(
            final ObjectNode monsterNode, final List<ItemDropInputData> itemDrops) {
