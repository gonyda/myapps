 ...
 *
 * <p>최대레벨 미만의 캐릭터가 임의의 경험치를 획득할 때, 최종 경험치가 다음 레벨 필요치 미만이고(최대레벨 도달 제외), 획득 레벨 수가 실제 증가 레벨과 일치하며, 초기
 * 경험치 + 획득량 = 레벨업 소비 합 + 최종 경험치가 성립함을 검증한다.
 *
...
class GainExperienceLevelUpPropertyTest {
    ...
    void should_conserveExperience_when_gainingExperience(
            @ForAll("levels") final int initialLevel, @ForAll("amounts") final long amount) {
        // Given: 초기 레벨에서 유효한 경험치를 가진 캐릭터
        final long requiredForInitial = experiencePolicy.requiredForNext(initialLevel);
        final long initialExp =
                requiredForInitial > 1
        ...
        if (finalLevel == MAX_LEVEL) {
            ...
            assertThat(finalExp).as("최대레벨 도달 시 잔여 경험치는 0이어야 한다").isEqualTo(0L);
        } else {
            // 일반 레벨업: 경험치 정확히 보존
            assertThat(initialExp + amount)
                    .as("경험치 총량이 보존되어야 한다")
