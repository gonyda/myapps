 ...
 *
 * <p>lastRebirthAt 값과 현재 시각의 관계에 따라 환생 가능 여부를 올바르게 판정하고,
 * 쿨다운 활성 상태에서 환생을 시도하면 {@code CooldownActive}를 반환하며
 * 캐릭터 상태가 불변인지 검증한다.
 *
...
class RebirthCooldownPropertyTest {
    ...
    /**
     * lastRebirthAt이 null이면 환생 가능(available=true)이고,
     * 환생 이력이 없음(everRebirthed=false)인지 검증한다.
     *
     * @param level 현재 레벨 (1~100)
     */
    @Property(tries = 100)
    void should_beAvailable_when_lastRebirthAtIsNull(
        ...
        assertThat(status.available())
                .as("lastRebirthAt이 null이면 환생 가능해야 한다")
                .isTrue();

        // Then: 환생 이력 없음
        assertThat(status.everRebirthed())
                .as("lastRebirthAt이 null이면 환생 이력이 없어야 한다")
                .isFalse();
    }
    ...
    void should_beAvailable_when_elapsedIs24HoursOrMore(
            @ForAll("hoursAgoAtLeast24") final int hoursAgo,
            @ForAll("levels") final int level) {
        ...
        // Then: 환생 가능
        assertThat(status.available())
                .as("24시간 이상 경과하면 환생 가능해야 한다")
                .isTrue();

        // Then: 환생 이력 있음
        assertThat(status.everRebirthed())
                .as("lastRebirthAt이 존재하면 환생 이력이 있어야 한다")
                .isTrue();
    }

    /**
     * 마지막 환생으로부터 24시간 미만이면 환생 불가(available=false)이고,
     * 남은 쿨다운 시간이 올바른지 검증한다.
     *
    ...
    void should_notBeAvailable_when_elapsedIsLessThan24Hours(
        ...
        assertThat(status.remaining())
                .as("남은 쿨다운은 24h - 경과 시간이어야 한다")
                .isEqualTo(expectedRemaining);
    }

    /**
     * 환생이 불가능한 상태에서 rebirth를 호출하면 CooldownActive가 반환되고,
     * 캐릭터 상태가 전혀 변하지 않는지 검증한다.
     *
     * @param totalMinutesAgo 경과 시간 (1~1439분, 즉 24시간 미만)
     * @param level           현재 레벨 (1~100)
     * @param talent          재능 유형
