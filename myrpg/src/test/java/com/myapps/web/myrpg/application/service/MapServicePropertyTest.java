 ...
 * 맵 파싱 라운드트립 프로퍼티 테스트.
 *
 * <p>유효한 맵 그래프를 JSON으로 직렬화한 뒤 파싱하여
 * 모든 노드의 id/name/type/좌표/links가 보존되고
 * NodeType이 원본 type 문자열에 대응하는지 검증한다.
 *
...
class MapServicePropertyTest {
    ...
    /**
     * 유효한 맵 그래프를 JSON 직렬화 후 파싱하면 모든 노드의
     * id/name/type/좌표/links가 보존되고 NodeType이 대응하는지 검증한다.
     *
     ...
     * town/field/dungeon 및 미지 타입과 theme 유무를 포함한다.
     *
    ...
    private Arbitrary<List<MapNode>> buildGridGraph(final int gridSize) {
        ...
        return Combinators.combine(types.list().ofSize(gridSize * gridSize), hasTheme.list().ofSize(gridSize * gridSize))
