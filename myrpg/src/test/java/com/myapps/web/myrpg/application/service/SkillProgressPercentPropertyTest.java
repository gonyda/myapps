 ...
 *
 * <p>공식: {@code (int)((min(usageCurrent/usageRequired, 1.0) + min(killCurrent/killRequired, 1.0)) / 2.0 * 100)}
 *
 * <p>검증 속성:
 * <ul>
 *   <li>결과가 [0, 100] 범위에 있다</li>
 *   <li>usage ≥ req AND kill ≥ req → 결과 100</li>
 *   <li>usage == 0 AND kill == 0 → 결과 0</li>
 *   <li>usage == req AND kill == 0 → 결과 50</li>
 * </ul>
 *
...
class SkillProgressPercentPropertyTest {
     ...
     * @return 진행률 퍼센트
     */
    private int calculateProgressPercent(final int usageCurrent,
    ...
    void should_returnBetween0And100_when_anyValidInput(
        ...
        final int result = calculateProgressPercent(usageCurrent, killCurrent, usageRequired, killRequired);

        assertThat(result)
                .as("진행률은 항상 [0, 100] 범위에 있어야 한다")
                .isBetween(0, FULL_PROGRESS_PERCENT);
    }
    ...
    void should_return0_when_bothCountsAreZero(
        ...
        assertThat(result)
                .as("usage == 0 AND kill == 0이면 진행률은 0이어야 한다")
                .isEqualTo(0);
    }
    ...
    void should_return50_when_onlyUsageMet(
        ...
        assertThat(result)
                .as("usage == req AND kill == 0이면 진행률은 50이어야 한다")
                .isEqualTo(50);
    }
