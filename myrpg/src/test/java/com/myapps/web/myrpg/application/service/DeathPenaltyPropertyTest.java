 ...
 *
 * <p>최대레벨 미만의 캐릭터에 사망 패널티를 적용할 때, 경험치가 {@code max(0, prev - floor(requiredForNext(level) ×
 * 0.10))}으로 설정되고, 레벨·누적레벨·재능은 불변이며, 최대레벨에서는 아무 변경이 없음을 검증한다.
 *
...
class DeathPenaltyPropertyTest {
    ...
    /**
     * 최대레벨 미만 캐릭터에 사망 패널티를 적용할 때 경험치가 올바르게 차감되고 레벨·누적레벨·재능이 불변인지 검증한다.
     *
     ...
     */
    @Property(tries = 100)
    void should_floorToZero_when_expLessThanPenalty(@ForAll("levels") final int level) {
        ...
        assertThat(progress.getExperience()).as("경험치가 패널티보다 적으면 0이어야 한다").isEqualTo(0L);

        // Then: 차감량은 초기 경험치 전부
        assertThat(result.experienceLost()).as("차감량은 초기 경험치 전부여야 한다").isEqualTo(initialExp);
    }
     ...
     */
    @Property(tries = 100)
    void should_noChange_when_atMaxLevel(@ForAll("talents") final TalentType talent) {
        ...
        // Then: 경험치 무변경
        assertThat(progress.getExperience()).as("최대레벨에서 경험치는 변하지 않아야 한다").isEqualTo(0L);

        // Then: 차감량 0
        assertThat(result.experienceLost()).as("최대레벨에서 차감량은 0이어야 한다").isEqualTo(0L);

        // Then: 레벨 불변
        assertThat(progress.getCurrentLevel()).as("최대레벨은 변하지 않아야 한다").isEqualTo(MAX_LEVEL);
    }
