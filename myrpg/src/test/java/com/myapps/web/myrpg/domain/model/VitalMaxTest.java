 ...
 *
 * <p>각 델타 헬퍼({@code withHpDelta}/{@code withMpDelta}/{@code withStaminaDelta})가 원본을 변경하지 않고 대상 필드만
 * 변경한 새 인스턴스를 반환하는지(음수 델타 포함) 확인하고, {@code TalentBonus}의 생성자와 {@code target()}/{@code perLevel()}
 * 접근자를 검증한다.
 *
