 ...
 *
 * <p>레벨업과 스킬 랭크업의 임의 시퀀스에 대해, {@code abilityPoints == (accumulatedLevel - 1) - totalConsumedAP}가 항상
 * 성립하는지 검증한다. 여기서 {@code totalConsumedAP}는 보유 스킬 각각에 대해 F에서 현재 랭크까지의 apCost 합이다.
 *
...
class SkillApInvariantPropertyTest {
    ...
    /**
     * 임의의 누적 레벨과 랭크업 횟수 시퀀스에 대해, 매 랭크업 후 AP 정합성 불변식이 성립하는지 검증한다.
     *
     * <p>생성 시나리오:
     *
     * <ol>
     *   <li>accumulatedLevel을 10~200 사이로 생성
     *   <li>초기 AP = accumulatedLevel - 1 (전량 사용 가능한 상태)
     *   <li>랭크업 가능한 횟수까지 순차적으로 랭크업 실행
     *   <li>매 랭크업 후 불변식 확인
     * </ol>
     *
    ...
    void should_maintain_ap_invariant_after_sequence_of_rankUps(
        ...
        for (int i = 0; i < rankUpCount; i++) {
            ...
            assertThat(result).as("랭크업 %d회차 성공 기대 (currentRank=%s)", i + 1, currentRank).isTrue();
            ...
            assertThat(progress.getAbilityPoints())
                    .as(
                            "AP 불변식 위반 (랭크업 %d회 후): AP=%d, expected=(accLv-1)-consumed=(%d-1)-%d=%d",
                            i + 1,
        ...
        assertThat(progress.getAbilityPoints())
                .as(
                        "최종 AP 불변식: AP=%d, accLv=%d, consumed=%d",
    ...
    void should_maintain_ap_invariant_across_multiple_skills(
        ...
        for (int i = 0; i < rankUpsPerSkill; i++) {
            ...
            // 중간 불변식 확인
            assertApInvariant(
                    progress, accumulatedLevel, totalConsumedAp, "스킬1 랭크업 " + (i + 1) + "회 후");
            ...
            // 중간 불변식 확인
            assertApInvariant(
                    progress, accumulatedLevel, totalConsumedAp, "스킬2 랭크업 " + (i + 1) + "회 후");
        }
    }
     ...
     */
    private void assertApInvariant(
            final CharacterProgress progress,
        ...
        assertThat(progress.getAbilityPoints())
                .as(
                        "AP 불변식 위반 (%s): AP=%d, expected=(accLv-1)-consumed=(%d-1)-%d=%d",
                        context,
                        progress.getAbilityPoints(),
                        accumulatedLevel,
                        totalConsumedAp,
