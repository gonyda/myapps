 ...
 *
 * <p>{@code defenseBlockRate}와 {@code defenseCounterRate}는 몬스터가 방어(DEFENSE) 행동을
 * 선택했을 때 적용되는 경감률(%)과 반격율(%)이다. 몬스터별 오버라이드가 가능하며,
 * 미지정 시 전역 기본값(경감 40% / 반격 30%)을 사용한다. 두 필드 모두 %이므로
 * 레벨과 무관하게 자동 스케일된다.
 *
...
public record Monster(
     ...
     *
     * <p>기존 12필드 호출부와의 하위 호환을 유지하기 위해 제공된다.
     * {@code monster.json}에서 두 필드가 미지정된 경우 이 생성자를 통해 인스턴스를 생성한다.
     *
     * @param id          몬스터 고유 식별자
     ...
     */
    public Monster(final String id, final String name, final MonsterType type,
                   final int level, final int maxHp, final int attackPower,
                   final int defense, final int critical, final long experience,
                   final GoldDrop goldDrop, final List<ItemDrop> itemDrops,
                   final List<String> lines) {
        this(id, name, type, level, maxHp, attackPower, defense, critical,
                experience, goldDrop, itemDrops, lines,
                DEFAULT_DEFENSE_BLOCK_RATE, DEFAULT_DEFENSE_COUNTER_RATE);
    }
