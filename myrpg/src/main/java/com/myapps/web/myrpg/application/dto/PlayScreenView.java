 ...
 *
 * <p>상단바, 미니맵, 전체지도, 상황 멘트, NPC 대화, 몬스터 조우,
 * 상호작용 목록, NPC/몬스터 행동 버튼, 행동 로그, 정보 팝업 등
 * 화면 렌더링에 필요한 모든 데이터를 하나로 묶어 제공한다.
 *
 * <p>NPC 대사 슬롯과 몬스터 대사 슬롯은 동시에 활성되지 않는다.
 * NPC 클릭 시 몬스터 슬롯이, 몬스터 클릭 시 NPC 슬롯이 비워진다(null).
 *
...
public record PlayScreenView(
     ...
     *
     * <p>기존 호출부(테스트 포함)가 몬스터 관련 필드를 명시하지 않고
     * 기존 10개 인자만으로 인스턴스를 생성할 수 있도록 유지한다.
     * 몬스터 슬롯 5개 필드는 모두 null로 설정된다.
     *
     * @param topBar       상단바 뷰 모델
     * @param minimap      미니맵 뷰 모델
     * @param fullMap      전체지도 뷰 모델
    ...
    public PlayScreenView(
        ...
        this(topBar, minimap, fullMap, ambience, npcName, npcDialogue,
                interactions, npcActions, null, null, null, null, null, logs, info);
    }
}
