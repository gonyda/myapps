 ...
 *
 * <p>STR, DEX, INT, Critical, DEF 다섯 가지 능력치를 정수로 보관한다. 이 record는 JPA 엔티티에 직접 매핑되지 않으며, 레벨·장비·스킬
 * 등으로부터 계산되어 조립된 결과를 표현하는 용도로만 사용한다.
 *
 ...
 */
public record Stats(int str, int dex, int intelligence, int critical, int defense) {
    ...
    public static Stats createDefault() {
        return new Stats(
                DEFAULT_STR, DEFAULT_DEX, DEFAULT_INTELLIGENCE, DEFAULT_CRITICAL, DEFAULT_DEFENSE);
    }
