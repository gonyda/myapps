 ...
 *
 * <p>딜스킬(NORMAL/HEAVY)은 랭크별 데미지 배율(%), 디펜스 스킬은 랭크별
 * 피해 경감률(%)과 반격 배율(%)을 제공한다. 보간 없이 카탈로그 맵에서
 * 직접 조회한다.
 *
 * <p>전투(7순위)가 실제 데미지·방어 계산에서 소비하며, 본 스펙(005)은
 * 조회 정책과 승급 모달 표시용까지만 구현한다.
 *
...
public class SkillDamagePolicy {
    ...
    public int multiplier(final DamageSkill skill, final SkillRank rank) {
        ...
        if (value == null) {
            throw new IllegalArgumentException(
                    "Rank " + rank.label() + " not found in multiplierByRank for skill: " + skill.id());
        }
        return value;
    }
    ...
    public int blockRate(final DefenseSkill skill, final SkillRank rank) {
        ...
        if (value == null) {
            throw new IllegalArgumentException(
                    "Rank " + rank.label() + " not found in blockRateByRank for skill: " + skill.id());
        }
        return value;
    }
    ...
    public int counterMultiplier(final DefenseSkill skill, final SkillRank rank) {
        ...
        if (value == null) {
            throw new IllegalArgumentException(
                    "Rank " + rank.label() + " not found in counterMultiplierByRank for skill: " + skill.id());
        }
        return value;
    }
}
