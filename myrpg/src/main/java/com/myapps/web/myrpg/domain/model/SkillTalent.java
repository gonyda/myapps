 ...
 * 스킬의 재능 분류를 정의하는 열거형.
 *
 * <p>각 상수는 대응하는 {@link TalentType}(또는 {@code null}), 자원 종류({@link ResourceKind}),
 * 랭크업 영구 보너스 대상({@link BonusTarget})을 보유한다.
 *
 * <ul>
 *   <li>{@link #matchingTalent()} — 재능 매칭 판정 소스. {@code COMMON}은 매칭 없음(empty).</li>
 *   <li>{@link #resourceKind()} — {@code MAGIC}은 MP, 그 외는 STAMINA.</li>
 *   <li>{@link #rankupStatTarget()} — 랭크업 시 영구 가산되는 스탯 대상.</li>
 * </ul>
 */
public enum SkillTalent {
    ...
    SkillTalent(final TalentType matchingTalent, final ResourceKind resourceKind,
     ...
     *
     * <p>{@code COMMON}은 매칭 재능이 없으므로 빈 {@code Optional}을 반환한다.
     * 전투(7순위)에서 재능 일치 +10% 판정의 소스로 사용된다.
     *
     ...
     *
     * <p>{@code MELEE}→{@link BonusTarget#STR}, {@code ARCHERY}→{@link BonusTarget#DEX},
     * {@code MAGIC}→{@link BonusTarget#INT}, {@code COMMON}→{@link BonusTarget#DEF}.
     *
     ...
     *
     * <p>상수명 비교는 대소문자를 구분한다(예: "MELEE"). {@code null}, 공백,
