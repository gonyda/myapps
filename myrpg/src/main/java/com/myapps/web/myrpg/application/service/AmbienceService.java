 ...
 * 상황 멘트 선택 서비스.
 *
 * <p>현재 시각으로부터 계절(Season)과 시간대(Time_Of_Day)를 산출하고, 노드의 테마에 맞는 멘트 후보를 선택합니다. 후보 선택 시 폴백 규칙을 적용하여 항상
 * 유효한 멘트를 반환합니다.
 */
@Service
public class AmbienceService {
    ...
    public AmbienceService(
            final ObjectMapper objectMapper, final Clock clock, final Random random) {
    ...
    void init() {
        ...
        try (InputStream inputStream = resource.getInputStream()) {
            ...
            throw new MapDataException("상황 멘트 JSON 파일 로딩 실패: " + AMBIENCE_JSON_PATH, exception);
        }
    }
     ...
     *
     * <p>테마·계절·시간대 조합의 후보에서 무작위로 선택하며, 후보가 없을 경우 폴백 규칙을 적용합니다.
     *
    ...
    String resolveTimeOfDay(final int hour) {
        for (final Map.Entry<String, AmbienceData.TimeBucket> entry :
                ambienceData.timeOfDay().entrySet()) {
    ...
    private List<String> findCandidates(
            final String theme, final String season, final String timeOfDay) {
    ...
    private List<String> findSeasonFallback(
            final Map<String, Map<String, List<String>>> themeMap, final String season) {
        final Map<String, List<String>> seasonMap = themeMap.get(season);
        if (seasonMap == null) {
            return List.of();
        }
