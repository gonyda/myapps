 ...
 *
 * <p>싱글 플레이어 구조에서 하나의 행만 유지하며, 캐릭터가 NPC 은행을 통해 골드를 입금하거나 출금할 때 사용된다. 은행 골드는 사망 패널티·환생에 영향받지 않는 안전
 * 보관소 역할을 한다.
 */
@Entity
@Table(name = "bank")
public class Bank {
     ...
     * 지정된 금액을 은행에 입금한다.
     *
     * <p>캐릭터 소지금에서 차감된 골드를 은행으로 이전할 때 사용한다. 금액이 0 이하이면 예외를 던진다.
     *
     ...
     * 지정된 금액을 은행에서 출금한다.
     *
     * <p>은행 보관 골드를 캐릭터 소지금으로 이전할 때 사용한다. 금액이 0 이하이면 {@link IllegalArgumentException}을, 보관 골드를 초과하면
     * {@link InsufficientGoldException}을 던지고 잔액을 변경하지 않는다.
     *
    ...
    public void withdraw(final long amount) {
        ...
        if (amount > this.gold) {
            throw new InsufficientGoldException("은행 잔액 부족: 출금 요청 " + amount + ", 보관 " + this.gold);
        }
        this.gold -= amount;
    }
}
