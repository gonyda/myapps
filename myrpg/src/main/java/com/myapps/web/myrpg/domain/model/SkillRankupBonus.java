 ...
 *
 * <p>규칙: 각 보유 스킬에 대해 {@code rank.order() × 재능별 주 스탯 +1}을 누적한다.
 * 가산 대상은 스탯 계열(STR/DEX/INT/DEF)뿐이며 HP/MP/Stamina/Critical에는 0을 부여한다.
 *
 * <p>예시:
 * <ul>
 *   <li>windmill(MELEE, rank A=order 5) → STR += 5</li>
 *   <li>7개 스킬 전부 MASTER(order 15): MELEE×2 + ARCHERY×2 + MAGIC×2 + COMMON×1
 *       → STR +30, DEX +30, INT +30, DEF +15</li>
 * </ul>
 *
 * <p>이 클래스는 Spring 빈이 아닌 순수 정책 객체로 사용된다.
 * 서비스 계층이 카탈로그 조회 함수를 주입하여 도메인↔응용 계층 의존을 차단한다.
 *
 * @see BonusTarget#STR
 * @see BonusTarget#DEX
 * @see BonusTarget#INT
...
public class SkillRankupBonus {
     ...
     */
    public Stats sum(final List<CharacterSkill> owned,
                     final Function<String, Optional<Skill>> skillLookup) {
