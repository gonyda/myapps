 ...
 *
 * <p>캐릭터의 현재 노드에서 미니맵·전체지도·상황 멘트·상호작용(NPC+몬스터)· 행동 로그·정보 팝업을 모아 {@link PlayScreenView}를 생성한다.
 *
 * <p>플레이 화면 진입({@code GET /})과 전투 종료 후 화면 복원에서 동일한 뷰를 재사용하기 위해 분리되었다. 전투 종료 시 이 뷰를 사용하지 않으면 상호작용
...
public class NodeViewAssembler {
     ...
     *
     * <p>현재 노드의 NPC·몬스터 목록으로 상호작용 버튼을 구성하고, 환생 상태를 조회하여 정보 팝업을 조립한 뒤 뷰를 반환한다.
     *
