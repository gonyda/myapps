 ...
 *
 * <p>골드는 {@link GoldDrop}의 {@code [min, max]} 범위에서 결정적 공식으로 산출하고,
 * 아이템은 각 {@link ItemDrop}의 확률 판정을 거쳐 수량 범위 내에서 추첨한다.
 * 순수 계산만 수행하며, 실제 지급(골드 가산·인벤토리 적재)은 이 서비스의 책임 밖이다.
 */
@Service
public class MonsterRewardService {
     ...
     *
     * <p>순수 함수로서, 동일 입력에 대해 항상 동일 결과를 반환한다.
     * 결과는 항상 {@code [goldDrop.min(), goldDrop.max()]} 범위에 속한다.
     *
     ...
     *
     * <p>골드는 필수로 산출되며, 각 아이템 드랍은 {@code chancePercent} 확률 판정을
     * 통과한 경우에만 {@code [minQuantity, maxQuantity]} 범위에서 수량이 결정된다.
     *
     * <p>이 메서드는 드랍 테이블을 추첨하여 결과만 반환한다. 실제 골드
     * 가산({@code CharacterProgress.gainGold})과 인벤토리 적재(OwnedItem 생성/수량 증가)는
     * 6순위(전투 완료 후) 구현에서 이 결과를 소비하여 처리한다. 6순위에서는 인벤토리
     * 획득 API를 신설하고 이 메서드의 반환값을 입력으로 전달한다.
     *
