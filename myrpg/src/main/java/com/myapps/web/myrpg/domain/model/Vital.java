 ...
 *
 * <p>이 record는 JPA 엔티티에 직접 매핑되지 않는다. 저장되는 값은 현재값({@code current})뿐이며, 최대값({@code max})은 레벨·장비·스킬
 * 등으로부터 계산되어 조립된다. 표시·로직 모두 정수이며, 현재값이 0이어도 보정하지 않는다.
 *
 * @param current 현재값
 * @param max 최대값
 */
public record Vital(int current, int max) {
