 ...
 *
 * <p>플레이어가 필드 노드로 이동할 때, 해당 노드에 배치된 몬스터 중 하나가 선공(preemptive strike)을 걸 확률을 판정한다. 선공 확률은 전 맵 고정
 * 5%이며, 발동 시 배치 몬스터 중 균등 무작위로 1마리를 선택한다.
 */
@Service
public class MonsterEncounterService {
     ...
     *
     * <p>순수 함수로, {@code roll < 5} 이면 발동(true)을 반환한다. roll은 [0, 99] 범위를 기대한다.
     *
     ...
     *
     * <p>이 메서드는 선공 판정과 대상 몬스터 선택만 수행한다. 실제 전투 진입(POST /battle/start)과 선공 턴 처리(몬스터가 먼저 공격)는 6순위(전투
     * 시스템) 구현에서 이 반환값을 소비하여 처리한다. 현재는 클라이언트가 alert('몬스터 선공 발동')을 표시하는 신호로만 사용되며, 6순위에서 alert를 전투 진입
     * POST로 교체한다.
     *
