 ...
 *
 * <p>미니맵 격자에서 하나의 맵 노드 위치, 이름(라벨)과 연결 정보를 담는다. {@code gridColumn}과 {@code gridRow}는 CSS {@code
 * grid-column}/{@code grid-row}에 직접 매핑된다.
 *
 * @param nodeId 노드 고유 식별자
 * @param name 노드 이름 (하단 라벨 표시용)
 * @param gridColumn CSS grid-column 값 (중심 노드 = 3, 범위 1~5)
