 ...
 * 힐러집 치료를 처리하는 컨트롤러.
 *
 * <p>팝업 없이 {@code POST /heal} 단일 호출로 100골드를 소모하고,
 * 상단바 최대치(레벨·재능 + 장비 바이탈 보너스)를 기준으로
 * HP/MP/스태미나를 풀회복한다.
 *
 * <p>응답은 {@code 200 OK}이며, 클라이언트(myrpg.js)가
 * {@code alert("치료되었습니다!")}와 {@code refreshTopBar()}를 수행한다.
 */
@Controller
@RequestMapping("/heal")
public class HealController {
     ...
     */
    public HealController(final CharacterService characterService,
     ...
     *
     * <p>상단바 최대치와 동일한 기준(레벨·재능 + 장비 바이탈 보너스)으로
     * {@link VitalMax}를 산출하여 {@code fullRecover(vitalMax)}를 호출한다.
     * 골드 부족 시 {@code InsufficientGoldException}이 발생하여
    ...
    public ResponseEntity<Void> heal() {
        ...
        final VitalMax baseVitalMax = statProgression.vitalMaxFor(
