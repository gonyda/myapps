 ...
 * 은행 통합 금고 관리 서비스.
 *
 * <p>싱글 플레이어 구조에서 유일한 은행 행을 로드하거나 기본 생성하고,
 * 캐릭터 소지금과 은행 골드 간의 입금/출금을 단일 트랜잭션으로 처리한다.
 * 최소 1골드, 상한 없음, 수수료 없음의 정책을 따른다.
 */
@Service
public class BankService {
     ...
     *
     * <p>{@code CharacterService.loadOrCreateDefault()} 선례를 따른다.
     * 싱글 플레이어 구조에서 은행 행은 항상 1개만 존재한다.
     *
     ...
     *
     * <p>단일 트랜잭션 내에서 {@code ch.spendGold(amount)}로 소지금을 차감한 뒤
     * {@code bank.deposit(amount)}로 은행 골드를 증가시킨다.
     * 소지금이 부족하면 {@code InsufficientGoldException}이 던져지고
     * 트랜잭션이 롤백되어 양쪽 모두 변경되지 않는다.
     *
     ...
     *
     * <p>단일 트랜잭션 내에서 {@code bank.withdraw(amount)}로 은행 골드를 차감한 뒤
     * {@code ch.gainGold(amount)}로 소지금을 증가시킨다.
     * 은행 잔액이 부족하면 {@code InsufficientGoldException}이 던져지고
     * 트랜잭션이 롤백되어 양쪽 모두 변경되지 않는다.
     *
