 ...
 *
 * <p>각 랭크에서 다음 랭크로 승급하기 위해 필요한 사용 횟수·막타 처치 수와
 * AP 비용을 상수 테이블로 보유한다. {@code MASTER}(최고 랭크)에는 승급이 없으므로
 * 빈 값을 반환한다.
 *
 * <p>요구치와 AP 비용은 랭크 순서({@link SkillRank#order()})가 오름에 따라
 * 단조 증가하며, F→Master 전체 AP 소모 합계는 200이다.
 *
