 ...
 *
 * <p>애플리케이션 기동 시 {@code classpath:data/item.json}을 1회 파싱하여
 * 불변 {@code List<Item>}을 구성하고, ID별·전체 목록 조회 기능을 제공합니다.
 * 데이터 무결성 위반 시 {@link ItemDataException}을 발생시켜 기동을 실패시킵니다.
 */
@Service
public class ItemCatalogService {
    ...
    void init() {
        try (InputStream inputStream = new ClassPathResource(ITEM_JSON_PATH).getInputStream()) {
            ...
            throw new ItemDataException(
                    "아이템 JSON 파일 로딩 실패: " + ITEM_JSON_PATH, exception);
        }
    }
    ...
    private Item parseItemNode(final JsonNode itemNode, final Set<String> ids) {
        ...
        final ItemType itemType = ItemType.fromString(typeString)
                .orElseThrow(() -> new ItemDataException(
                        "아이템 '" + id + "'의 type '" + typeString + "'을(를) 변환할 수 없습니다."));
    ...
    private PotionItem parsePotionItem(final JsonNode itemNode, final String id,
                                       final String name, final Integer buyPrice) {
        final int healHp = extractRequiredInt(itemNode, "healHp", id);
        return new PotionItem(id, name, healHp, buyPrice);
    }

    private EquipmentItem parseEquipmentItem(final JsonNode itemNode, final String id,
                                             final String name, final ItemType itemType,
                                             final Integer buyPrice) {
        final String kindString = extractRequiredField(itemNode, "kind");
        final EquipmentKind kind = EquipmentKind.fromString(kindString)
                .orElseThrow(() -> new ItemDataException(
                        "아이템 '" + id + "'의 kind '" + kindString + "'을(를) 변환할 수 없습니다."));

        if (!itemNode.has("maxDurability")) {
            throw new ItemDataException(
                    "장비 아이템 '" + id + "'에 필수 필드 'maxDurability'가 누락되었습니다.");
        }
    ...
    private List<EquipBonus> parseBonuses(final JsonNode itemNode, final String itemId) {
        ...
        if (!bonusesNode.isArray()) {
            throw new ItemDataException(
                    "아이템 '" + itemId + "'의 'bonuses' 필드가 배열이 아닙니다.");
        }
    ...
    private BonusTarget parseBonusTarget(final String targetString, final String itemId) {
        try {
            ...
            throw new ItemDataException(
                    "아이템 '" + itemId + "'의 bonuses.target '" + targetString
                            + "'을(를) 변환할 수 없습니다.");
        }
    }
