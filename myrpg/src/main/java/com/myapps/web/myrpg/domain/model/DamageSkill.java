 ...
 *
 * <p>각 랭크({@link SkillRank})에 대한 1히트당 데미지 배율(%)을 {@link #multiplierByRank()}로 보유하며,
 * 랭크가 오를수록 배율은 단조 증가한다. 맵에는 반드시 16개 랭크 키가 모두 포함되어야 한다.
 *
 * <p>{@link #hitCount()}는 스킬 사용 시 반복 타격 횟수(기본 1)를 나타내며,
 * {@link #critBonus()}는 이 스킬 사용 시 크리티컬 확률에 가산되는 보너스(기본 0, 상한 100)를 나타낸다.
 *
 * @param id               스킬 고유 식별자
 * @param label            표시용 라벨
...
public record DamageSkill(
     ...
     * @param description      스킬 설명 문자열
     */
    public DamageSkill(final String id, final String label, final SkillType type,
                       final SkillTalent talent, final int resourceCost,
                       final Map<SkillRank, Integer> multiplierByRank, final String description) {
