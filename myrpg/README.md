...
| **오후 (`afternoon`)** | `12:00 ~ 16:00` | 로열 사파이어 블루 & 정오의 태양 (`left: 50%`) | 정중앙 최고점(남중)에 위치한 찬란한 정오 태양 |
| **황혼 (`late-afternoon`)** | `16:00 ~ 19:00` | 황혼 인디고·노을 크림슨 & 지는 석양 (`left: 80%`) | 서쪽(우측)으로 저물며 붉게 물드는 저녁 노을빛 태양 |
| **밤 (`night`)** | `19:00 ~ 24:00` | 옵시디언 나이트 & 은빛 보름달 (`left: 22%`) | 동쪽(좌측)에서 떠오르는 은빛 보름달 & 별무리 |
