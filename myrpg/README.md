...
10. [기획 백로그 & 시스템 로드맵](#10-기획-백로그--시스템-로드맵)
...
```mermaid
graph TD
    ALBY["알비 던전 입구<br>(0, -1)"] --- TIR["티르코네일 (마을)<br>(0, 0)"]
    TIR --- DUG_N["두갈드 아일 북부 (필드)<br>(0, 1)"]
    TIR --- EAST["동쪽 언덕 (필드)<br>(1, 0)"]
    EAST --- CIAR["키아 던전 입구<br>(2, 0)"]
    DUG_N --- DUG_S["두갈드 아일 남부 (필드)<br>(0, 2)"]
    DUG_S --- DUN["던바튼 (마을)<br>(0, 3)"]
    DUN --- RABBIE["라비 던전 입구<br>(1, 3)"]
    DUN --- WEST["서쪽 숲 (필드)<br>(-1, 3)"]
```
...
### 8.1. 몬스터 6종 데이터 테이블 (`monster.json`)

| 몬스터명 (ID) | 타입 | 레벨 | HP | 공격력 | 방어력 | 치명타 | 처치 경험치 | 드랍 골드 | 드랍 아이템 |
|---|---|---|---|---|---|---|---|---|---|
| **너구리** (`raccoon`) | 일반 | Lv 1 | 45 | 42 | 3 | 3.0% (30) | 22 EXP | 6 ~ 16 G | 생명력 30 포션 (15%) |
| **거미** (`spider`) | 일반 | Lv 2 | 65 | 48 | 4 | 3.0% (30) | 30 EXP | 8 ~ 20 G | 생명력 30 포션 (10%) |
| **붉은거미** (`red-spider`) | 일반 | Lv 3 | 80 | 50 | 5 | 3.0% (30) | 40 EXP | 12 ~ 25 G | 생명력 30 포션 (15%) |
| **고블린** (`goblin`) | 일반 | Lv 3 | 70 | 54 | 3 | 3.0% (30) | 42 EXP | 15 ~ 30 G | 생명력 30 포션 (20%) |
| **검은거미** (`black-spider`) | 일반 | Lv 4 | 100 | 56 | 6 | 4.0% (40) | 55 EXP | 20 ~ 45 G | 생명력 30 포션 (25%) |
...
현재 예정되어 있는 차기 개발 백로그입니다 (`memory-bank/activeContext.md` 및 `docs/todo.md` 참조):

1. **인게임 가상 시간 시스템 (In-Game Time)**
   - Day N, 00:00~23:59 가상 시간 영속화
   - 맵 노드 이동 시 시간 경과 연동 (예: 노드당 10~30분 소요)
2. **야간 시간대 기습 시스템 (Night Ambush)**
   - 야간 시간대(22:00 ~ 06:00) 필드 이동 시 몬스터 기습 확률 대폭 상향 (5% $\rightarrow$ **50%**)
3. **마비노기식 필드 캠프파이어 & 휴식 (Campfire & Rest)**
   - 장작 아이템과 캠프파이어 설치를 통한 야영
   - 불 주위에서 휴식 시 HP/MP/스태미나 점진적 회복 및 버프 효과
4. **신규 던전 및 인챈트 시스템 (Ciar/Rabbie & Enchant)**
   - 키아 던전(골렘), 라비 던전(서큐버스) 인스턴스 오픈
   - 장비에 접두/접미 주문서를 부여하는 인챈트 및 인스턴스 보너스 시스템
