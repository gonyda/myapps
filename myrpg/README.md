...
7. [월드 맵, 인게임 시간 & NPC 도감](#7-월드-맵-인게임-시간--npc-도감)
...
MyRPG는 **마비노기(Mabinogi)의 클래식한 감성과 시스템(가위바위보 심리전 전투, 스킬 랭크업, 장비 내구도 및 수리, 24시간 환생, 무기 세트 I/II 스왑, 인게임 24시간 일주 궤적, 절차적 던전 탐험 등)**을 웹 환경에 맞춰 턴제로 재해석한 RPG 게임입니다.
...
- **원클릭 퀵 로그인 프리셋**:
  - `bbsk` (일반 테스터): 초기 레벨 1 및 기본 장비/스킬 세팅
  - `admin` (관리자/개발용): 초기 스킬 35종 전체 F랭크 일괄 습득 상태
- **신규 캐릭터 기본 지급품**:
  1. **기본 스킬 4종 (F랭크)**:
     - 근접: `베기(slash)` F랭크
  ...
  5. **초기 상태**:
     - 레벨: Lv 1, 누적 레벨: 1, 경험치: 0, 소지금: 0G, 인게임 시각: 08:00 (오전), 시작 위치: 티르코네일 (`tir-chonaill`)
...
### 6.1. 상점 시스템 & 착용 장비 2단 비교 팝업 (`ShopService`)
- **상점 판매가 공식**:
...
- **상점 장비 착용 2단 비교 팝업 (`ShopBuyItemView`)**:
  - 상점 구매 목록에서 돋보기(🔍) 클릭 시, **상단 [상점 물품 스펙] vs 하단 [현재 착용 중인 동일 슬롯 장비 스펙]**을 상하 2단으로 직관적으로 비교 표시합니다.
  - 현재 해당 부위가 미착용 상태이거나 인벤토리/은행 등 타 팝업에서는 단독 상세 뷰로 유연하게 동작합니다.
...
### 7.2. 인게임 시간 시스템 (In-Game Time) & 6대 시간대 앰비언스 (`TimeOfDay`)

#### 1. 인게임 시간 흐름 및 영속화 (`CharacterProgress`)
- 캐릭터 진행 데이터에 가상 24시간 시계(`inGameMinutes`: 0~1439분, 기본 08:00 시작)가 영속화됩니다.
- **시간 경과 규칙**:
  - **필드 노드 이동 시**: **+15분** 경과
  - **던전 방 이동 시**: **+5분** 경과
  - 장애물/적 봉쇄로 이동 실패(Blocked) 시: **0분** (시간 불변)
- **24시간 디지털 시계**: 좌측 사이드바 최하단에 `HH:mm` 형식의 디지털 시계 위젯이 상시 표시되며 맵 이동 시 실시간 동기화됩니다.

#### 2. 6대 시간대별 앰비언트 스카이 & 천체 24시간 일주 궤적
인게임 시각에 따라 메인 스테이지(`.center`)와 전투 화면(`.battle-view`)의 하늘 배경 그라디언트와 천체(해/달)가 동쪽(좌측)에서 서쪽(우측)으로 자연스럽게 일주합니다:

| 시간대 (`TimeOfDay`) | 구간 | 하늘 테마 & 천체 궤적 | 연출 상세 |
|---|---|---|---|
| **심야 (`late-night`)** | `00:00 ~ 05:00` | 딥 미드나잇 퍼플 & 창백한 달 (`left: 78%`) | 서쪽으로 기울어지는 달, 밤하늘 별빛 레이어 (`stars-layer`) |
| **새벽 (`dawn`)** | `05:00 ~ 08:00` | 로즈 인디고 & 피어오르는 아침 해 (`left: 18%`) | 동쪽(좌측)에서 떠오르는 붉은 여명빛 태양 |
| **오전 (`morning`)** | `08:00 ~ 12:00` | 맑은 청록 네이비 & 상승하는 골드 태양 (`left: 32%`) | 좌측에서 중천으로 상승하는 화사한 골드 태양 |
| **오후 (`afternoon`)** | `12:00 ~ 18:00` | 로열 사파이어 블루 & 정오의 태양 (`left: 50%`) | 정중앙 최고점(남중)에 위치한 찬란한 정오 태양 |
| **황혼 (`late-afternoon`)** | `18:00 ~ 20:00` | 황혼 인디고·노을 크림슨 & 지는 석양 (`left: 80%`) | 서쪽(우측)으로 저물며 붉게 물드는 저녁 노을빛 태양 |
| **밤 (`night`)** | `20:00 ~ 24:00` | 옵시디언 나이트 & 은빛 보름달 (`left: 22%`) | 동쪽(좌측)에서 떠오르는 은빛 보름달 & 별무리 |

#### 3. 테마 $\times$ 계절 $\times$ 시간대 실시간 상황 멘트 풀 (`AmbienceService`)
- 맵 노드의 테마(마을, 필드, 던전, 묘지 등)와 현재 계절(봄/여름/가을/겨울), 6대 시간대를 조합하여 상단 상황 멘트 슬림 바에 서정적인 멘트와 시간대 이모지가 실시간 출력됩니다.
